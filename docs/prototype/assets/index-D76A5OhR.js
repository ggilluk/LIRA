var M=Object.defineProperty;var A=(a,e,r)=>e in a?M(a,e,{enumerable:!0,configurable:!0,writable:!0,value:r}):a[e]=r;var o=(a,e,r)=>A(a,typeof e!="symbol"?e+"":e,r);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))t(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const i of n.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&t(i)}).observe(document,{childList:!0,subtree:!0});function r(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerPolicy&&(n.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?n.credentials="include":s.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function t(s){if(s.ep)return;s.ep=!0;const n=r(s);fetch(s.href,n)}})();class F{constructor(){o(this,"statuses",new Map);o(this,"listeners",new Set)}register(e,r,t="idle",s){this.statuses.set(e,{id:e,label:r,state:t,detail:s}),this.notify()}update(e,r,t,s){const n=this.statuses.get(e);n&&(this.statuses.set(e,{...n,state:r,detail:t,progress:s}),this.notify())}get(e){return this.statuses.get(e)}all(){return[...this.statuses.values()]}subscribe(e){return this.listeners.add(e),e(this.all()),()=>{this.listeners.delete(e)}}notify(){const e=this.all();for(const r of this.listeners)r(e)}}class P{constructor(e=[]){o(this,"domains",new Map);for(const r of e)this.add(r)}add(e){this.domains.set(e.name,e)}get(e){return this.domains.get(e)}all(){return[...this.domains.values()]}roots(){return this.all().filter(e=>e.parentName===void 0)}children(e){return this.all().filter(r=>r.parentName===e)}ancestryOf(e){const r=[];let t=this.get(e);for(;t!==void 0;)r.unshift(t),t=t.parentName!==void 0?this.get(t.parentName):void 0;return r}}class V{constructor(e,r="LIRA"){o(this,"unsubscribe");this.board=e,this.title=r}mount(e){var r;this.ensureStyles(),(r=this.unsubscribe)==null||r.call(this),this.unsubscribe=this.board.subscribe(t=>{e.innerHTML=this.renderScreen(t)})}waitFor(...e){return new Promise(r=>{const t=i=>i==="done"||i==="error",s=i=>{const l=i.filter(c=>e.includes(c.id));l.length===e.length&&l.every(c=>t(c.state))&&(n(),r())},n=this.board.subscribe(s)})}destroy(){var e;(e=this.unsubscribe)==null||e.call(this),this.unsubscribe=void 0}renderScreen(e){return`
    ...
    `}renderStep(e){return`
      <div class="loading-step state-${e.state}">
        <span class="loading-step-icon">${z[e.state]}</span>
        <span class="loading-step-label">${f(e.label)}</span>
        <span class="loading-step-detail">${f(e.detail??O[e.state])}</span>
      </div>
    `}ensureStyles(){if(document.getElementById(w))return;const e=document.createElement("style");e.id=w,e.textContent=q,document.head.appendChild(e)}}const O={"not-ported":"Not ported yet",idle:"Waiting…",running:"Working…",done:"Ready",error:"Failed"},z={"not-ported":"–",idle:"○",running:'<span class="loading-spinner"></span>',done:"✓",error:"✕"};function f(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const w="lira-loading-screen-styles",q=`
.loading-screen {
...
`,j=300;class U{constructor(){o(this,"events",[]);o(this,"listeners",new Set)}append(e){this.events.push(e),this.events.length>j&&this.events.shift(),this.notify()}all(){return[...this.events]}subscribe(e){return this.listeners.add(e),e(this.all()),()=>{this.listeners.delete(e)}}notify(){const e=this.all();for(const r of this.listeners)r(e)}}class H{constructor(e,r){this.source=e,this.sink=r}info(e){this.emit("info",e)}warn(e){this.emit("warn",e)}error(e){this.emit("error",e)}emit(e,r){this.sink({timestamp:Date.now(),level:e,source:this.source,message:r})}}class _{constructor(e,r=[],t){o(this,"unsubscribeStatus");o(this,"unsubscribeLog");o(this,"collapsed",!1);o(this,"logCollapsed",!1);o(this,"latestStatuses",[]);o(this,"latestEvents",[]);o(this,"container");this.board=e,this.actions=r,this.logBoard=t}mount(e){var r,t;this.container=e,this.ensureStyles(),(r=this.unsubscribeStatus)==null||r.call(this),(t=this.unsubscribeLog)==null||t.call(this),e.addEventListener("click",s=>this.handleClick(s)),this.unsubscribeStatus=this.board.subscribe(s=>{this.latestStatuses=s,this.paint()}),this.logBoard&&(this.unsubscribeLog=this.logBoard.subscribe(s=>{this.latestEvents=s,this.paint()}))}destroy(){var e,r;(e=this.unsubscribeStatus)==null||e.call(this),this.unsubscribeStatus=void 0,(r=this.unsubscribeLog)==null||r.call(this),this.unsubscribeLog=void 0}paint(){this.container&&(this.container.innerHTML=this.renderPanel())}handleClick(e){const r=e.target.closest("[data-action]");if(!(!r||r.disabled)){if(r.dataset.action==="toggle")this.collapsed=!this.collapsed,this.paint();else if(r.dataset.action==="toggle-log")this.logCollapsed=!this.logCollapsed,this.paint();else if(r.dataset.action==="run"){const t=this.actions.find(s=>s.id===r.dataset.actionId);t==null||t.onClick()}}}renderPanel(){const e=this.latestStatuses,r=e.filter(t=>t.state==="running"||t.state==="done").length;return`
      <div class="service-status-panel ${this.collapsed?"collapsed":""}">
        <button type="button" class="service-status-header" data-action="toggle" aria-expanded="${!this.collapsed}">
          ...
          <span class="service-status-chevron">${y}</span>
        </button>
    ...
    `}renderRow(e){const r=this.actions.find(s=>s.id===e.id),t=e.progress!==void 0?`<div class="service-status-progress"><div class="service-status-progress-fill" style="width:${Math.round(e.progress*100)}%"></div></div>`:"";return`
      <div class="service-status-row-group">
        <div class="service-status-row state-${e.state}">
          ...
          <span class="service-status-pill">${W[e.state]}</span>
    ...
    `}renderLogPanel(){const e=this.latestEvents;return`
      <div class="service-log-panel ${this.logCollapsed?"collapsed":""}">
        ...
        <div class="service-log-rows">
          ${e.length===0?'<div class="service-log-empty">No events yet.</div>':[...e].reverse().map(r=>this.renderLogRow(r)).join("")}
        </div>
      </div>
    `}renderLogRow(e){return`
      <div class="service-log-row level-${e.level}">
        <span class="service-log-time">${Y(e.timestamp)}</span>
        <span class="service-log-level-pill">${K[e.level]}</span>
        <span class="service-log-source">${m(e.source)}</span>
        <span class="service-log-message">${m(e.message)}</span>
      </div>
    `}ensureStyles(){if(document.getElementById(x))return;const e=document.createElement("style");e.id=x,e.textContent=J,document.head.appendChild(e)}}const W={"not-ported":"Not ported",idle:"Idle",running:"Running",done:"Running",error:"Error"},K={info:"Info",warn:"Warn",error:"Error"};function m(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Y(a){return new Date(a).toLocaleTimeString(void 0,{hour12:!1})}const y='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6l4 4 4-4"/></svg>',x="lira-service-status-styles",J=`
...
`;var p=(a=>(a[a.NOUN=0]="NOUN",a[a.VERB=1]="VERB",a[a.ADJECTIVE=2]="ADJECTIVE",a[a.ADVERB=3]="ADVERB",a[a.PRONOUN=4]="PRONOUN",a[a.DETERMINER=5]="DETERMINER",a[a.PREPOSITION=6]="PREPOSITION",a[a.CONJUNCTION=7]="CONJUNCTION",a[a.INTERJECTION=8]="INTERJECTION",a[a.NUMERAL=9]="NUMERAL",a[a.AUXILIARY=11]="AUXILIARY",a[a.PROPER_NOUN=12]="PROPER_NOUN",a[a.SYMBOL=13]="SYMBOL",a[a.PUNCTUATION=14]="PUNCTUATION",a[a.OTHER=15]="OTHER",a))(p||{});const G={[p[p.NOUN]]:"#3B6EA5",[p[p.PROPER_NOUN]]:"#274472",[p[p.VERB]]:"#B2542D",[p[p.ADJECTIVE]]:"#7A5CA6",[p[p.ADVERB]]:"#B08900",[p[p.PRONOUN]]:"#5B7B6F",[p[p.DETERMINER]]:"#6E7B8B",[p[p.PREPOSITION]]:"#7B6E5B",[p[p.CONJUNCTION]]:"#6B7280",[p[p.AUXILIARY]]:"#5B6E8B",[p[p.INTERJECTION]]:"#C2544B",[p[p.NUMERAL]]:"#4B8A7B",[p[p.SYMBOL]]:"#8A8A8A",[p[p.PUNCTUATION]]:"#9A9A9A",[p[p.OTHER]]:"#7A7A7A"},g={VALID:"#2B6E63",UNRESOLVED:"#B08900",INVALID:"#B2542D"},k=["A meaning is a representation.","The word over the meaning.","The use is a state.","The word wants to use the meaning.","The meaning and the word perceive the state."],$="lira-sentence-reader-styles";class X{constructor(e){o(this,"container");o(this,"reading",!1);o(this,"requestToken",0);o(this,"documentResult");o(this,"expandedNodes",new Set);o(this,"selectedKey");o(this,"detailCache",new Map);o(this,"detailToken",0);this.client=e}mount(e){this.container=e,this.ensureStyles(),e.innerHTML=this.renderShell(),this.wire();const r=e.querySelector(".lira-sr-textarea");r!=null&&r.value&&this.read(r.value)}destroy(){this.container=void 0}wire(){const e=this.container;if(!e)return;const r=e.querySelector(".lira-sr-textarea"),t=e.querySelector(".lira-sr-read-btn");r&&t&&(t.addEventListener("click",()=>void this.read(r.value)),r.addEventListener("keydown",n=>{(n.metaKey||n.ctrlKey)&&n.key==="Enter"&&this.read(r.value)})),e.querySelectorAll(".lira-sr-example").forEach(n=>{n.addEventListener("click",()=>{const i=n.dataset.example??"";r&&(r.value=i),this.read(i)})});const s=e.querySelector(".lira-sr-tree");s==null||s.addEventListener("click",n=>this.handleTreeClick(n)),s==null||s.addEventListener("keydown",n=>this.handleTreeKeydown(n))}handleTreeClick(e){const r=e.target,t=r.closest('[data-action="toggle-node"]');if(t){this.toggleNode(t.dataset.node??"");return}const s=r.closest('.lira-tree-row[data-kind="sentence"]');s&&this.selectSentenceNode(s.dataset.node??"")}handleTreeKeydown(e){if(e.key!=="Enter"&&e.key!==" ")return;const r=e.target;if(r.closest('[data-action="toggle-node"]'))return;const t=r.closest('.lira-tree-row[data-kind="sentence"]');t&&(e.preventDefault(),this.selectSentenceNode(t.dataset.node??""))}toggleNode(e){if(!e||!this.documentResult)return;const r=!this.expandedNodes.has(e);if(r?this.expandedNodes.add(e):this.expandedNodes.delete(e),r&&E(e)&&!this.detailCache.has(e)){this.selectSentenceNode(e);return}this.renderTreeInPlace()}renderTreeInPlace(){var r;if(!this.documentResult)return;const e=(r=this.container)==null?void 0:r.querySelector(".lira-sr-tree");e&&(e.innerHTML=this.renderTree(this.documentResult))}async selectSentenceNode(e){const r=this.documentResult,t=E(e);if(!r||!t)return;const s=r.blocks[t.blockIndex];if(!s||s.blockKind!=="paragraph")return;const n=s.sentences[t.sentenceIndex];if(!n)return;this.selectedKey=e,this.expandedNodes.add(`b${t.blockIndex}`),this.expandedNodes.add(e),this.renderTreeInPlace();const i=this.detailCache.get(e);if(i){this.renderDetail(i);return}this.setPanelPlaceholder(".lira-sr-predicted","Loading…"),this.setPanelPlaceholder(".lira-sr-winner-panel","Loading…"),this.setPanelPlaceholder(".lira-sr-trace","Loading…");const l=++this.detailToken,c=this.isLearningEnabled();try{const u=await this.client.read(n.text,c,!0);if(l!==this.detailToken||!this.container)return;this.detailCache.set(e,u),this.renderTreeInPlace(),this.renderDetail(u)}catch(u){if(l!==this.detailToken||!this.container)return;this.setError(u instanceof Error?u.message:String(u))}}isLearningEnabled(){var r;const e=(r=this.container)==null?void 0:r.querySelector(".lira-sr-learning-toggle");return(e==null?void 0:e.checked)??!0}async read(e){const r=e.trim();if(!r||this.reading)return;const t=++this.requestToken,s=this.isLearningEnabled();this.reading=!0,this.setBusy(!0),this.setError(void 0);try{const n=await this.client.readDocument(r,s);if(t!==this.requestToken||!this.container)return;this.renderDocument(n)}catch(n){if(t!==this.requestToken||!this.container)return;this.setError(n instanceof Error?n.message:String(n))}finally{t===this.requestToken&&(this.reading=!1,this.setBusy(!1))}}setBusy(e){var t;const r=(t=this.container)==null?void 0:t.querySelector(".lira-sr-read-btn");r&&(r.disabled=e,r.textContent=e?"Reading…":"Read")}setLearningStatus(e){var s;const r=(s=this.container)==null?void 0:s.querySelector(".lira-sr-learning-status");if(!r)return;if(!e||!e.enabled){r.textContent="Learning off";return}const t=e.recordedThisRead>0?` (+${e.recordedThisRead})`:"";r.textContent=`Learning: ${e.totalObservations} observation${e.totalObservations===1?"":"s"}${t}`}setError(e){var t;const r=(t=this.container)==null?void 0:t.querySelector(".lira-sr-error");r&&(r.textContent=e??"",r.style.display=e?"block":"none")}renderDocument(e){this.documentResult=e.document,this.detailCache.clear(),this.expandedNodes=new Set(["doc"]),this.selectedKey=void 0,this.setLearningStatus(e.learning);const r=Q(e.document);if(r){this.selectSentenceNode(r);return}this.renderTreeInPlace(),this.setPanelPlaceholder(".lira-sr-predicted","No sentences found in this text."),this.setPanelPlaceholder(".lira-sr-winner-panel","No sentences found in this text."),this.setPanelPlaceholder(".lira-sr-trace","No sentences found in this text.")}renderDetail(e){var n,i,l;const r=(n=this.container)==null?void 0:n.querySelector(".lira-sr-predicted"),t=(i=this.container)==null?void 0:i.querySelector(".lira-sr-winner-panel"),s=(l=this.container)==null?void 0:l.querySelector(".lira-sr-trace");r&&(r.innerHTML=this.renderPredicted(e.predicted,e.words)),t&&(t.innerHTML=this.renderWinner(e.predicted,e.trace)),s&&(s.innerHTML=this.renderTrace(e.trace))}setPanelPlaceholder(e,r){var s;const t=(s=this.container)==null?void 0:s.querySelector(e);t&&(t.innerHTML=`<div class="lira-sr-placeholder">${d(r)}</div>`)}ensureStyles(){if(document.getElementById($))return;const e=document.createElement("style");e.id=$,e.textContent=ne,document.head.appendChild(e)}renderShell(){return`
      <div class="lira-sr">
        <div class="lira-sr-input-card">
          <textarea class="lira-sr-textarea" placeholder="e.g. A meaning is a representation.">${d(k[0])}</textarea>
          ...
          <div class="lira-sr-examples">
            ${k.map(e=>`<button type="button" class="lira-sr-example" data-example="${d(e)}">${d(e)}</button>`).join("")}
          </div>
        </div>
    ...
    `}renderTree(e){const r=this.expandedNodes.has("doc"),t=g[e.validation]??"#7A7A7A",s=e.blocks.filter(c=>c.blockKind==="paragraph").length,n=e.blocks.length-s,i=e.blocks.reduce((c,u)=>c+(u.blockKind==="paragraph"?u.errors.length:0),0),l=[`${s} paragraph${s===1?"":"s"}`,n?`${n} heading${n===1?"":"s"}`:"",i?`${i} error${i===1?"":"s"}`:""].filter(Boolean).join(", ");return`
      <ul class="lira-tree-root">
        <li class="lira-tree-node">
          <div class="lira-tree-row" data-node="doc" data-kind="document">
            ...
            ${r?"":`<span class="lira-tree-summary">${d(l)}</span>`}
          </div>
          ${r?`<ul>${e.blocks.map((c,u)=>this.renderBlockNode(c,u)).join("")}</ul>`:""}
        </li>
        ...
        <li class="lira-tree-node lira-tree-leaf">
          <div class="lira-tree-row" data-node="${t}" data-kind="heading">
            ...
            <span class="lira-tree-label">${d(v(e.text,48))}</span>
          </div>
        </li>`;const s=this.expandedNodes.has(t),n=g[e.validation]??"#7A7A7A",i=[`${e.sentences.length} sentence${e.sentences.length===1?"":"s"}`,e.errors.length?`${e.errors.length} error${e.errors.length===1?"":"s"}`:""].filter(Boolean).join(", ");return`
      <li class="lira-tree-node">
        <div class="lira-tree-row" data-node="${t}" data-kind="paragraph">
          ${b(t,s)}
          <span class="lira-tree-dot" style="background:${n}"></span>
          <span class="lira-tree-label">Paragraph ${r+1}</span>
          ${s?"":`<span class="lira-tree-summary">${d(i)}</span>`}
        </div>
        ${s?`<ul>${e.sentences.map((l,c)=>this.renderSentenceNode(l,t,c)).join("")}</ul>`:""}
      </li>`}renderSentenceNode(e,r,t){const s=`${r}s${t}`,n=g[e.validation]??"#7A7A7A",i=this.expandedNodes.has(s),l=this.detailCache.get(s),c=i?`<ul>${l?l.predicted.clauses.map((u,D)=>this.renderClauseNode(u,`${s}c${D}`)).join(""):'<li class="lira-tree-node"><div class="lira-tree-row"><span class="lira-tree-spacer"></span><span class="lira-tree-summary">Loading…</span></div></li>'}</ul>`:"";return`
      <li class="lira-tree-node">
        <div class="lira-tree-row ${this.selectedKey===s?"selected":""}" data-node="${s}" data-kind="sentence" role="button" tabindex="0">
          ...
          <span class="lira-tree-snippet">${d(v(e.text,40))}</span>
          ${e.errors.length?`<span class="lira-tree-error-count">${e.errors.length}</span>`:""}
        </div>
        ${c}
      </li>`}renderClauseNode(e,r,t){const s=g[e.validation]??"#7A7A7A",i=[["subject",e.subject],["predicate",e.predicate],["object",e.object],["complement",e.complement],...e.modifiers.map(l=>["modifier",l])].filter(l=>l[1]!==null).map(([l,c],u)=>R(c)?this.renderClauseNode(c,`${r}r${u}`,l):this.renderPhraseNode(c,`${r}r${u}`,l)).join("");return`
      <li class="lira-tree-node">
        <div class="lira-tree-row" data-node="${r}" data-kind="clause">
          <span class="lira-tree-spacer"></span>
          <span class="lira-tree-dot" style="background:${s}"></span>
          ${t?`<span class="lira-tree-role-pill">${d(t)}</span>`:""}
          <span class="lira-tree-label">Clause</span>
          <span class="lira-tree-summary">${d(e.clauseType??"?")}</span>
          <span class="lira-tree-snippet">${d(v(e.text,40))}</span>
        </div>
        ${i?`<ul>${i}</ul>`:""}
      </li>`}renderPhraseNode(e,r,t){const s=g[e.validation]??"#7A7A7A",n=e.nestedPhrases.map((i,l)=>this.renderPhraseNode(i,`${r}n${l}`)).join("");return`
      <li class="lira-tree-node">
        <div class="lira-tree-row" data-node="${r}" data-kind="phrase">
          <span class="lira-tree-spacer"></span>
          <span class="lira-tree-dot" style="background:${s}"></span>
          ${t?`<span class="lira-tree-role-pill">${d(t)}</span>`:""}
        ...
        ${n?`<ul>${n}</ul>`:""}
      </li>`}renderWinner(e,r){const t=e.clauses[0],n=(t?[["subject",t.subject],["predicate",t.predicate],["object",t.object],["complement",t.complement],...t.modifiers.map(l=>["modifier",l])]:[]).map(([l,c])=>B(l,c)).join(""),i=r.length?`<div class="lira-sr-winner-positions">
          ${r.map(l=>`
      ...
      ${n?`<div class="lira-sr-winner-roles">${n}</div>`:""}
      ${i}
    `}renderPredicted(e,r){return`${`
      ...
      <div class="lira-sr-position">
        <div class="lira-sr-position-head">
          ...
          <span class="lira-sr-faint">${d(r)}</span>
        ...
        ${e.attempts.map(s=>this.renderAttempt(s)).join("")}
          ...
          <div class="lira-sr-completion ${t.isWinner?"winner":""}">
            ...
            "${d(t.text)}" — ${d(t.validation)}, confidence ${t.confidence.toFixed(2)}
            ${te(t.tokens)}
      ...
      <div class="lira-sr-attempt ${e.startMatch?"":"rejected"}">
        ...
        ${r}
      </div>`}}function R(a){return"clauseType"in a}function B(a,e){if(!e)return"";if(R(e)){const r=[["subject",e.subject],["predicate",e.predicate],["object",e.object],["complement",e.complement],...e.modifiers.map(t=>["modifier",t])];return`
      <div class="lira-sr-winner-role-row">
        <span class="lira-sr-winner-role-label">${d(a)}</span>
        <span class="lira-sr-mono">"${d(e.text)}"</span>
        <span class="lira-sr-faint">embedded clause · ${d(e.clauseType??"?")}</span>
      </div>
      <div class="lira-sr-winner-roles lira-sr-winner-roles-nested">${r.map(([t,s])=>B(t,s)).join("")}</div>`}return`
    <div class="lira-sr-winner-role-row">
      <span class="lira-sr-winner-role-label">${d(a)}</span>
      <span class="lira-sr-mono">"${d(e.text)}"</span>
      <span class="lira-sr-faint">${d(e.phraseType??"?")}</span>
    </div>`}function b(a,e){return`<button type="button" class="lira-tree-toggle" data-action="toggle-node" data-node="${a}" aria-expanded="${e}">${ae}</button>`}function v(a,e){const r=a.trim();return r.length>e?`${r.slice(0,e-1)}…`:r}function E(a){const e=/^b(\d+)s(\d+)$/.exec(a);if(e)return{blockIndex:Number(e[1]),sentenceIndex:Number(e[2])}}function Q(a){for(let e=0;e<a.blocks.length;e+=1){const r=a.blocks[e];if(r.blockKind!=="paragraph")continue;const t=r.sentences.findIndex(s=>s.errors.length>0);if(t>=0)return`b${e}s${t}`}for(let e=0;e<a.blocks.length;e+=1){const r=a.blocks[e];if(r.blockKind==="paragraph"&&r.sentences.length>0)return`b${e}s0`}}function Z(a){if(!a.length)return'<div class="lira-sr-empty">No words to show.</div>';let e="";return a.forEach((r,t)=>{const s=/^[.,!?;:]+$/.test(r.text);t>0&&!s&&(e+=" "),e+=ee(r,t)}),`<div class="lira-sr-sentence">${e}</div>`}function ee(a,e){return`<span class="lira-sr-word${a.resolved?"":" lira-sr-word-unfound"}" tabindex="0" data-word-index="${e}">${d(a.text)}${re(a)}</span>`}function re(a){if(!a.resolved)return`<span class="lira-sr-word-tooltip"><span class="tt-title">${d(a.text)}</span><span class="tt-meta">Not found in the Common Vocabulary Cache</span></span>`;const e=[a.validation??"UNRESOLVED",a.phraseType??"no phrase"],r=[a.partOfSpeech??"?",`conf ${a.confidence!==null?a.confidence.toFixed(2):"—"}`];return`<span class="lira-sr-word-tooltip"><span class="tt-title">${d(a.text)}</span><span class="tt-meta">${e.map(d).join(" · ")}</span><span class="tt-meta">${r.map(d).join(" · ")}</span></span>`}function C(a){return`<span class="lira-sr-badge" style="background:${g[a]??"#7A7A7A"}">${d(a)}</span>`}function T(a,e){return`
    <span class="lira-sr-pos-chip" style="background:${G[e]??"#7A7A7A"}">
      <span class="w">${d(a)}</span>
      <span class="p">${d(e)}</span>
    </span>`}function te(a){return a.length?`<div class="lira-sr-trace-tokens">${a.map(r=>T(r.text,r.partOfSpeech??"UNKNOWN")).join("")}</div>`:""}function se(a){return a.length?`
    ...
    </div>`:""}function d(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const ae='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6l4 4 4-4"/></svg>',ne=`
...
`,S="Common",I=[{id:"vocabulary",label:"Vocabulary",available:!0},{id:"linguistics",label:"Linguistics",available:!0},{id:"knowledge",label:"Knowledge",available:!1}],L="lira-portal-shell-styles",N="lira-vocabulary-fragment-styles";class ie{constructor(e,r,t,s,n={}){o(this,"mode");o(this,"mobileScreen","browse");o(this,"selectedName");o(this,"selectedComponent","vocabulary");o(this,"treeCollapsed",!1);o(this,"title");o(this,"container");o(this,"serviceStatusView");o(this,"sentenceReaderView");o(this,"logBoard",new U);o(this,"loggersBySource",new Map);o(this,"renderToken",0);o(this,"currentVocabularyDomainName");var i;this.registry=e,this.vocabularyClient=r,this.statusBoard=s,this.title=n.title??"LIRA",this.mode=typeof window<"u"&&window.matchMedia("(max-width: 720px)").matches?"mobile":"desktop",this.selectedName=(i=this.registry.roots()[0])==null?void 0:i.name,this.serviceStatusView=new _(s,[],this.logBoard),this.sentenceReaderView=new X(t),this.vocabularyClient.onLog((l,c,u)=>{this.loggerFor(c)[l](u)}),this.statusBoard.subscribe(l=>{const c=l.find(u=>u.id==="vocabulary");c&&this.updateVocabToolbar(c)}),this.vocabularyClient.onDomainUpdated(l=>{this.registry.add(l),this.render()}),this.searchWordsBridge(),this.searchPhrasesBridge(),this.searchSensesBridge(),this.searchRelationshipsBridge(),this.searchLexicalRelationshipsBridge(),this.resolveHierarchyBridge()}searchWordsBridge(){document.addEventListener("lira-search-words",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.searchWords(this.currentVocabularyDomainName,{wordId:r.wordId,word:r.word,gloss:r.gloss,definition:r.definition,pos:r.pos,domainLabel:r.domain,rootWordsOnly:r.rootWordsOnly,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-search-words-result",{detail:{requestId:r.requestId,words:t.words,totalMatches:t.totalMatches}}))})})}searchPhrasesBridge(){document.addEventListener("lira-search-phrases",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.searchPhrases(this.currentVocabularyDomainName,{word:r.word,gloss:r.gloss,definition:r.definition,pos:r.pos,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-search-phrases-result",{detail:{requestId:r.requestId,phrases:t.phrases,totalMatches:t.totalMatches}}))})})}searchSensesBridge(){document.addEventListener("lira-search-senses",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.searchSenses(this.currentVocabularyDomainName,{word:r.word,gloss:r.gloss,definition:r.definition,pos:r.pos,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-search-senses-result",{detail:{requestId:r.requestId,senses:t.senses,totalMatches:t.totalMatches}}))})})}searchRelationshipsBridge(){document.addEventListener("lira-search-relationships",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.searchRelationships(this.currentVocabularyDomainName,{wordId:r.wordId,query:r.query,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-search-relationships-result",{detail:{requestId:r.requestId,relationships:t.relationships,totalMatches:t.totalMatches}}))})})}searchLexicalRelationshipsBridge(){document.addEventListener("lira-search-lexical-relationships",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.searchLexicalRelationships(this.currentVocabularyDomainName,{wordId:r.wordId,query:r.query,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-search-lexical-relationships-result",{detail:{requestId:r.requestId,relationships:t.relationships,totalMatches:t.totalMatches}}))})})}resolveHierarchyBridge(){document.addEventListener("lira-resolve-hierarchy",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.resolveHierarchy(this.currentVocabularyDomainName,{kind:r.kind,wordId:r.wordId,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-resolve-hierarchy-result",{detail:{requestId:r.requestId,...t}}))})})}mount(e){this.container=e,document.title=this.title,this.ensureStyles(),e.addEventListener("click",r=>this.handleClick(r)),this.render()}handleClick(e){const r=e.target.closest("[data-action]");if(!r||r.disabled)return;const t=r.dataset.action;t==="select"?(this.selectedName=r.dataset.domain,this.mode==="mobile"&&(this.mobileScreen="view"),this.render()):t==="mode"?(this.mode=r.dataset.mode,this.mode==="mobile"&&(this.mobileScreen=this.selectedName?"view":"browse"),this.render()):t==="back"?(this.mobileScreen="browse",this.render()):t==="component"?(this.selectedComponent=r.dataset.component,this.render()):t==="toggle-tree"?(this.treeCollapsed=!this.treeCollapsed,this.render()):t==="seed-wordnet"?this.vocabularyClient.seedWordNet(S):t==="seed-common-vocabulary"&&this.vocabularyClient.seedCommonVocabulary(S)}renderVocabToolbar(){return`<div class="portal-vocab-toolbar">${this.vocabToolbarInner(this.statusBoard.get("vocabulary"))}</div>`}vocabToolbarInner(e){const r=(e==null?void 0:e.state)==="running",t=e==null?void 0:e.progress;return`
    ...
    `}updateVocabToolbar(e){var t;const r=(t=this.container)==null?void 0:t.querySelector(".portal-vocab-toolbar");r&&(r.innerHTML=this.vocabToolbarInner(e))}loggerFor(e){let r=this.loggersBySource.get(e);return r||(r=new H(e,t=>this.logBoard.append(t)),this.loggersBySource.set(e,r)),r}ensureStyles(){if(document.getElementById(L))return;const e=document.createElement("style");e.id=L,e.textContent=ve,document.head.appendChild(e)}ensureFragmentStyles(e){if(document.getElementById(N))return;const r=document.createElement("style");r.id=N,r.textContent=e,document.head.appendChild(r)}render(){if(!this.container)return;const e=this.selectedName?this.registry.get(this.selectedName):void 0;this.renderToken++;const r=this.mode==="desktop"&&this.treeCollapsed?"tree-collapsed":"";this.container.innerHTML=`
    ...
    `;const t=this.container.querySelector(".portal-service-status");t&&this.serviceStatusView.mount(t),e&&this.selectedComponent==="vocabulary"?this.loadView(e):e&&this.selectedComponent==="linguistics"&&this.loadLinguisticsView()}loadLinguisticsView(){if(!this.container)return;const e=this.container.querySelector(".portal-fragment-mount");e&&this.sentenceReaderView.mount(e)}async loadView(e){const r=++this.renderToken;this.setViewStatus("Loading Vocabulary…");let t;try{t=await this.vocabularyClient.renderDomain(e.name)}catch(n){if(r!==this.renderToken)return;this.setViewStatus(`Couldn't load this Domain's Vocabulary view: ${n instanceof Error?n.message:String(n)}`);return}if(r!==this.renderToken||!this.container)return;this.ensureFragmentStyles(t.style);const s=this.container.querySelector(".portal-fragment-mount");if(s){s.innerHTML=t.body,this.currentVocabularyDomainName=e.name;const n=document.createElement("script");n.textContent=`(function () {
${t.script}
})();`,s.appendChild(n)}this.setViewStatus(void 0)}setViewStatus(e){var t;const r=(t=this.container)==null?void 0:t.querySelector(".portal-view-status");r&&(r.textContent=e??"",r.style.display=e?"block":"none")}renderTopbar(e){const r=`
      <div class="portal-mode-toggle" role="group" aria-label="Layout">
        <button type="button" data-action="mode" data-mode="desktop" class="${this.mode==="desktop"?"active":""}">${ue} Desktop</button>
        <button type="button" data-action="mode" data-mode="mobile" class="${this.mode==="mobile"?"active":""}">${he} Mobile</button>
      </div>`;if(this.mode==="mobile")return this.mobileScreen==="view"&&e?`
          <div class="portal-topbar">
            <button type="button" class="portal-back" data-action="back" aria-label="Back to Domains">${pe}</button>
            <span class="portal-topbar-title">${h(e.name)}</span>
            ${r}
          </div>`:`<div class="portal-topbar"><span class="portal-topbar-title">All Domains</span>${r}</div>`;const t=e?this.registry.ancestryOf(e.name):[];return`<div class="portal-topbar"><nav class="portal-breadcrumb">${[`<span class="crumb-root">${ce} All Domains</span>`,...t.map(n=>`<span>${h(n.name)}</span>`)].join(`<span class="crumb-sep">${de}</span>`)}</nav>${r}</div>`}renderBody(e){return this.mode==="desktop"?`${this.renderTree()}${this.renderViewPane(e)}`:this.mobileScreen==="browse"?`<div class="portal-tree portal-tree--mobile">${this.renderTreeRows(0)}</div>`:this.renderViewPane(e,!0)}renderTree(){return this.treeCollapsed?`
        <nav class="portal-tree portal-tree--collapsed">
          <button type="button" class="portal-tree-toggle" data-action="toggle-tree" title="Expand Domains" aria-label="Expand Domains" aria-expanded="false">${ge}</button>
        </nav>`:`
      <nav class="portal-tree">
        <div class="portal-tree-label">
          <span>Domains</span>
          <button type="button" class="portal-tree-toggle" data-action="toggle-tree" title="Collapse Domains" aria-label="Collapse Domains" aria-expanded="true">${me}</button>
        </div>
        ${this.renderTreeRows(0)}
      </nav>`}renderTreeRows(e,r){return(e===0?this.registry.roots():this.registry.children(r??"")).map(s=>{const n=this.registry.children(s.name),i=s.name===this.selectedName;return`
          <div class="portal-tree-row depth-${e} ${i?"selected":""}" data-action="select" data-domain="${h(s.name)}">
            ${n.length>0?le:'<span class="chev-spacer"></span>'}
            ${oe}
            <span class="name">${h(s.name)}</span>
            <span class="count">${s.wordCount.toLocaleString()}</span>
          </div>
          ${n.length>0?this.renderTreeRows(e+1,s.name):""}
      ...
      <div class="portal-component-switcher" role="tablist" aria-label="UI Component">
        ...
        `).join("")}
      </div>`}renderViewPane(e,r=!1){const t=this.renderComponentSwitcher(),s='<div class="portal-service-status"></div>',n=this.selectedComponent==="vocabulary"?this.renderVocabToolbar():"";if(!e)return`
        <div class="portal-view ${r?"portal-view--full":""}">
          ${t}
          ${n}
          <div class="portal-view-empty">Select a Domain to continue.</div>
          ${s}
        </div>`;const i=I.find(c=>c.id===this.selectedComponent),l=i!=null&&i.available?`
      ...
      <div class="portal-view ${r?"portal-view--full":""}">
        ...
        ${l}
        ${s}
      </div>`}}function h(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const oe='<svg class="i-folder" viewBox="0 0 16 16" fill="currentColor"><path d="M1.5 3.5A1 1 0 0 1 2.5 2.5h3.6l1.2 1.4H13.5A1 1 0 0 1 14.5 5v7A1 1 0 0 1 13.5 13h-11a1 1 0 0 1-1-1v-8.5z"/></svg>',le='<svg class="i-chev" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6l4 4 4-4"/></svg>',de='<svg class="i-chev-right" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 3l5 5-5 5"/></svg>',ce='<svg class="i-home" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M2 8l6-5 6 5M4 7v6h8V7"/></svg>',pe='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 3L5 8l5 5"/></svg>',ue='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="1.5" y="2.5" width="13" height="9" rx="1"/><path d="M6 13.5h4"/></svg>',he='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="4.5" y="1.5" width="7" height="13" rx="1.4"/></svg>',me='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 3L5 8l5 5"/></svg>',ge='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 3l5 5-5 5"/></svg>',ve=`
.portal-shell {
...
`;class fe{constructor(){o(this,"worker");o(this,"statusListeners",new Set);o(this,"domainUpdateListeners",new Set);o(this,"logListeners",new Set);o(this,"readyResolvers",[]);o(this,"pendingRenders",new Map);o(this,"pendingSearches",new Map);o(this,"pendingPhraseSearches",new Map);o(this,"pendingSenseSearches",new Map);o(this,"pendingRelationshipSearches",new Map);o(this,"pendingLexicalRelationshipSearches",new Map);o(this,"pendingHierarchyResolutions",new Map);this.worker=new Worker(new URL(""+new URL("vocabulary_worker-M7wHvVz6.js",import.meta.url).href,import.meta.url),{type:"module"}),this.worker.addEventListener("message",e=>{this.handleMessage(e.data)})}onStatus(e){return this.statusListeners.add(e),()=>{this.statusListeners.delete(e)}}onLog(e){return this.logListeners.add(e),()=>{this.logListeners.delete(e)}}init(){return new Promise(e=>{this.readyResolvers.push(e),this.post({type:"init"})})}linkPort(e){this.worker.postMessage({type:"link-port",port:e},[e])}renderDomain(e){const r=`${e}-${Math.random().toString(36).slice(2)}`;return new Promise((t,s)=>{this.pendingRenders.set(r,{resolve:t,reject:s}),this.post({type:"render",requestId:r,domain:e})})}seedWordNet(e){this.post({type:"seed-wordnet",domain:e})}seedCommonVocabulary(e){this.post({type:"seed-common-vocabulary",domain:e})}onDomainUpdated(e){return this.domainUpdateListeners.add(e),()=>{this.domainUpdateListeners.delete(e)}}searchWords(e,r){const t=`search-${e}-${Math.random().toString(36).slice(2)}`;return new Promise(s=>{this.pendingSearches.set(t,s),this.post({type:"search-words",requestId:t,domain:e,wordId:r.wordId,word:r.word,gloss:r.gloss,definition:r.definition,pos:r.pos,domainLabel:r.domainLabel,rootWordsOnly:r.rootWordsOnly,limit:r.limit})})}searchPhrases(e,r){const t=`phrase-search-${e}-${Math.random().toString(36).slice(2)}`;return new Promise(s=>{this.pendingPhraseSearches.set(t,s),this.post({type:"search-phrases",requestId:t,domain:e,word:r.word,gloss:r.gloss,definition:r.definition,pos:r.pos,limit:r.limit})})}searchSenses(e,r){const t=`sense-search-${e}-${Math.random().toString(36).slice(2)}`;return new Promise(s=>{this.pendingSenseSearches.set(t,s),this.post({type:"search-senses",requestId:t,domain:e,word:r.word,gloss:r.gloss,definition:r.definition,pos:r.pos,limit:r.limit})})}searchRelationships(e,r){const t=`rel-search-${e}-${Math.random().toString(36).slice(2)}`;return new Promise(s=>{this.pendingRelationshipSearches.set(t,s),this.post({type:"search-relationships",requestId:t,domain:e,wordId:r.wordId,query:r.query,limit:r.limit})})}searchLexicalRelationships(e,r){const t=`lexical-rel-search-${e}-${Math.random().toString(36).slice(2)}`;return new Promise(s=>{this.pendingLexicalRelationshipSearches.set(t,s),this.post({type:"search-lexical-relationships",requestId:t,domain:e,wordId:r.wordId,query:r.query,limit:r.limit})})}resolveHierarchy(e,r){const t=`hierarchy-${e}-${Math.random().toString(36).slice(2)}`;return new Promise(s=>{this.pendingHierarchyResolutions.set(t,s),this.post({type:"resolve-hierarchy",requestId:t,domain:e,kind:r.kind,wordId:r.wordId,limit:r.limit})})}post(e){this.worker.postMessage(e)}handleMessage(e){if(e.type==="status")for(const r of this.statusListeners)r(e.state,e.detail,e.progress);else if(e.type==="ready"){const r=this.readyResolvers.splice(0);for(const t of r)t(e.domains)}else if(e.type==="rendered"){const r=this.pendingRenders.get(e.requestId);r&&(this.pendingRenders.delete(e.requestId),r.resolve(e.fragment))}else if(e.type==="render-error"){const r=this.pendingRenders.get(e.requestId);r&&(this.pendingRenders.delete(e.requestId),r.reject(new Error(e.message)))}else if(e.type==="domain-updated")for(const r of this.domainUpdateListeners)r(e.domain);else if(e.type==="log")for(const r of this.logListeners)r(e.level,e.source,e.message,e.timestamp);else if(e.type==="search-words-result"){const r=this.pendingSearches.get(e.requestId);r&&(this.pendingSearches.delete(e.requestId),r({words:e.words,totalMatches:e.totalMatches}))}else if(e.type==="search-phrases-result"){const r=this.pendingPhraseSearches.get(e.requestId);r&&(this.pendingPhraseSearches.delete(e.requestId),r({phrases:e.phrases,totalMatches:e.totalMatches}))}else if(e.type==="search-senses-result"){const r=this.pendingSenseSearches.get(e.requestId);r&&(this.pendingSenseSearches.delete(e.requestId),r({senses:e.senses,totalMatches:e.totalMatches}))}else if(e.type==="search-relationships-result"){const r=this.pendingRelationshipSearches.get(e.requestId);r&&(this.pendingRelationshipSearches.delete(e.requestId),r({relationships:e.relationships,totalMatches:e.totalMatches}))}else if(e.type==="search-lexical-relationships-result"){const r=this.pendingLexicalRelationshipSearches.get(e.requestId);r&&(this.pendingLexicalRelationshipSearches.delete(e.requestId),r({relationships:e.relationships,totalMatches:e.totalMatches}))}else if(e.type==="resolve-hierarchy-result"){const r=this.pendingHierarchyResolutions.get(e.requestId);r&&(this.pendingHierarchyResolutions.delete(e.requestId),r({nodes:e.nodes,edges:e.edges,roots:e.roots,totalEdgeCount:e.totalEdgeCount,totalNodeCount:e.totalNodeCount,fellBack:e.fellBack,truncated:e.truncated}))}else e.type==="error"&&console.error("Vocabulary Service error:",e.message)}}class be{constructor(){o(this,"worker");o(this,"statusListeners",new Set);o(this,"readyResolvers",[]);o(this,"pendingReads",new Map);o(this,"pendingReadDocuments",new Map);this.worker=new Worker(new URL(""+new URL("linguistics_worker-rRqkDtIe.js",import.meta.url).href,import.meta.url),{type:"module"}),this.worker.addEventListener("message",e=>{this.handleMessage(e.data)})}onStatus(e){return this.statusListeners.add(e),()=>{this.statusListeners.delete(e)}}init(){return new Promise(e=>{this.readyResolvers.push(e),this.post({type:"init"})})}linkVocabularyPort(e){this.worker.postMessage({type:"link-vocabulary-port",port:e},[e])}read(e,r,t=!1){const s=`read-${Math.random().toString(36).slice(2)}`;return new Promise((n,i)=>{this.pendingReads.set(s,{resolve:n,reject:i}),this.post({type:"read",requestId:s,text:e,learningEnabled:r,skipLearning:t})})}readDocument(e,r){const t=`read-doc-${Math.random().toString(36).slice(2)}`;return new Promise((s,n)=>{this.pendingReadDocuments.set(t,{resolve:s,reject:n}),this.post({type:"read-document",requestId:t,text:e,learningEnabled:r})})}post(e){this.worker.postMessage(e)}handleMessage(e){if(e.type==="status")for(const r of this.statusListeners)r(e.state,e.detail);else if(e.type==="ready"){const r=this.readyResolvers.splice(0);for(const t of r)t(e.wordCount)}else if(e.type==="read-result"){const r=this.pendingReads.get(e.requestId);r&&(this.pendingReads.delete(e.requestId),r.resolve(e.result))}else if(e.type==="read-document-result"){const r=this.pendingReadDocuments.get(e.requestId);r&&(this.pendingReadDocuments.delete(e.requestId),r.resolve(e.result))}else if(e.type==="error"){if(e.requestId){const r=this.pendingReads.get(e.requestId);if(r){this.pendingReads.delete(e.requestId),r.reject(new Error(e.message));return}const t=this.pendingReadDocuments.get(e.requestId);if(t){this.pendingReadDocuments.delete(e.requestId),t.reject(new Error(e.message));return}}console.error("Linguistic Service error:",e.message)}}}function we(){const a=document.querySelector("#app");if(!a)return;a.style.height="100vh";const e=new F;e.register("vocabulary","Vocabulary Service","idle","Starting…"),e.register("linguistics","Linguistic Service","idle","Starting…"),e.register("knowledge","Knowledge Service","not-ported");const r=new V(e,"LIRA");r.mount(a);const t=new fe;t.onStatus((i,l,c)=>e.update("vocabulary",i,l,c));const s=new be;s.onStatus((i,l)=>e.update("linguistics",i,l));const n=new MessageChannel;t.linkPort(n.port1),s.linkVocabularyPort(n.port2),Promise.all([t.init(),s.init()]).then(([i])=>{r.destroy();const l=new P(i);new ie(l,t,s,e,{title:"LIRA"}).mount(a)}).catch(i=>{e.update("vocabulary","error",i instanceof Error?i.message:String(i))})}we();
