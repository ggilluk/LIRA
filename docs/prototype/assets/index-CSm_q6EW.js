var M=Object.defineProperty;var D=(a,e,r)=>e in a?M(a,e,{enumerable:!0,configurable:!0,writable:!0,value:r}):a[e]=r;var d=(a,e,r)=>D(a,typeof e!="symbol"?e+"":e,r);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))t(s);new MutationObserver(s=>{for(const n of s)if(n.type==="childList")for(const i of n.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&t(i)}).observe(document,{childList:!0,subtree:!0});function r(s){const n={};return s.integrity&&(n.integrity=s.integrity),s.referrerPolicy&&(n.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?n.credentials="include":s.crossOrigin==="anonymous"?n.credentials="omit":n.credentials="same-origin",n}function t(s){if(s.ep)return;s.ep=!0;const n=r(s);fetch(s.href,n)}})();class A{constructor(){d(this,"statuses",new Map);d(this,"listeners",new Set)}register(e,r,t="idle",s){this.statuses.set(e,{id:e,label:r,state:t,detail:s}),this.notify()}update(e,r,t,s){const n=this.statuses.get(e);n&&(this.statuses.set(e,{...n,state:r,detail:t,progress:s}),this.notify())}get(e){return this.statuses.get(e)}all(){return[...this.statuses.values()]}subscribe(e){return this.listeners.add(e),e(this.all()),()=>{this.listeners.delete(e)}}notify(){const e=this.all();for(const r of this.listeners)r(e)}}class P{constructor(e=[]){d(this,"domains",new Map);for(const r of e)this.add(r)}add(e){this.domains.set(e.name,e)}get(e){return this.domains.get(e)}all(){return[...this.domains.values()]}roots(){return this.all().filter(e=>e.parentName===void 0)}children(e){return this.all().filter(r=>r.parentName===e)}ancestryOf(e){const r=[];let t=this.get(e);for(;t!==void 0;)r.unshift(t),t=t.parentName!==void 0?this.get(t.parentName):void 0;return r}}class V{constructor(e,r="LIRA"){d(this,"unsubscribe");this.board=e,this.title=r}mount(e){var r;this.ensureStyles(),(r=this.unsubscribe)==null||r.call(this),this.unsubscribe=this.board.subscribe(t=>{e.innerHTML=this.renderScreen(t)})}waitFor(...e){return new Promise(r=>{const t=i=>i==="done"||i==="error",s=i=>{const o=i.filter(p=>e.includes(p.id));o.length===e.length&&o.every(p=>t(p.state))&&(n(),r())},n=this.board.subscribe(s)})}destroy(){var e;(e=this.unsubscribe)==null||e.call(this),this.unsubscribe=void 0}renderScreen(e){return`
      <div class="loading-screen">
        <div class="loading-box">
          <div class="loading-title">${v(this.title)}</div>
    ...
    `}renderStep(e){return`
      <div class="loading-step state-${e.state}">
        <span class="loading-step-icon">${F[e.state]}</span>
        <span class="loading-step-label">${v(e.label)}</span>
        <span class="loading-step-detail">${v(e.detail??O[e.state])}</span>
      </div>
    `}ensureStyles(){if(document.getElementById(w))return;const e=document.createElement("style");e.id=w,e.textContent=q,document.head.appendChild(e)}}const O={"not-ported":"Not ported yet",idle:"Waiting…",running:"Working…",done:"Ready",error:"Failed"},F={"not-ported":"–",idle:"○",running:'<span class="loading-spinner"></span>',done:"✓",error:"✕"};function v(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const w="lira-loading-screen-styles",q=`
.loading-screen {
...
`;class z{constructor(e,r=[]){d(this,"unsubscribe");d(this,"collapsed",!1);this.board=e,this.actions=r}mount(e){var r;this.ensureStyles(),(r=this.unsubscribe)==null||r.call(this),e.addEventListener("click",t=>this.handleClick(t,e)),this.unsubscribe=this.board.subscribe(t=>{e.innerHTML=this.renderPanel(t)})}destroy(){var e;(e=this.unsubscribe)==null||e.call(this),this.unsubscribe=void 0}handleClick(e,r){const t=e.target.closest("[data-action]");if(!(!t||t.disabled)){if(t.dataset.action==="toggle")this.collapsed=!this.collapsed,r.innerHTML=this.renderPanel(this.board.all());else if(t.dataset.action==="run"){const s=this.actions.find(n=>n.id===t.dataset.actionId);s==null||s.onClick()}}}renderPanel(e){const r=e.filter(t=>t.state==="running"||t.state==="done").length;return`
      <div class="service-status-panel ${this.collapsed?"collapsed":""}">
        <button type="button" class="service-status-header" data-action="toggle" aria-expanded="${!this.collapsed}">
          ...
          <span class="service-status-chevron">${U}</span>
        </button>
    ...
    `}renderRow(e){const r=this.actions.find(s=>s.id===e.id),t=e.progress!==void 0?`<div class="service-status-progress"><div class="service-status-progress-fill" style="width:${Math.round(e.progress*100)}%"></div></div>`:"";return`
      <div class="service-status-row-group">
        <div class="service-status-row state-${e.state}">
          ...
          <span class="service-status-name">${g(e.label)}</span>
          <span class="service-status-pill">${j[e.state]}</span>
          ${e.detail?`<span class="service-status-detail">${g(e.detail)}</span>`:""}
          ${r?`<button type="button" class="service-status-action" data-action="run" data-action-id="${g(r.id)}" ${e.state==="running"?"disabled":""}>${g(r.label)}</button>`:""}
        </div>
        ${t}
      </div>
    `}ensureStyles(){if(document.getElementById(y))return;const e=document.createElement("style");e.id=y,e.textContent=H,document.head.appendChild(e)}}const j={"not-ported":"Not ported",idle:"Idle",running:"Running",done:"Running",error:"Error"};function g(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const U='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6l4 4 4-4"/></svg>',y="lira-service-status-styles",H=`
...
`;var c=(a=>(a[a.NOUN=0]="NOUN",a[a.VERB=1]="VERB",a[a.ADJECTIVE=2]="ADJECTIVE",a[a.ADVERB=3]="ADVERB",a[a.PRONOUN=4]="PRONOUN",a[a.DETERMINER=5]="DETERMINER",a[a.PREPOSITION=6]="PREPOSITION",a[a.CONJUNCTION=7]="CONJUNCTION",a[a.INTERJECTION=8]="INTERJECTION",a[a.NUMERAL=9]="NUMERAL",a[a.AUXILIARY=11]="AUXILIARY",a[a.PROPER_NOUN=12]="PROPER_NOUN",a[a.SYMBOL=13]="SYMBOL",a[a.PUNCTUATION=14]="PUNCTUATION",a[a.OTHER=15]="OTHER",a))(c||{});const _={[c[c.NOUN]]:"#3B6EA5",[c[c.PROPER_NOUN]]:"#274472",[c[c.VERB]]:"#B2542D",[c[c.ADJECTIVE]]:"#7A5CA6",[c[c.ADVERB]]:"#B08900",[c[c.PRONOUN]]:"#5B7B6F",[c[c.DETERMINER]]:"#6E7B8B",[c[c.PREPOSITION]]:"#7B6E5B",[c[c.CONJUNCTION]]:"#6B7280",[c[c.AUXILIARY]]:"#5B6E8B",[c[c.INTERJECTION]]:"#C2544B",[c[c.NUMERAL]]:"#4B8A7B",[c[c.SYMBOL]]:"#8A8A8A",[c[c.PUNCTUATION]]:"#9A9A9A",[c[c.OTHER]]:"#7A7A7A"},m={VALID:"#2B6E63",UNRESOLVED:"#B08900",INVALID:"#B2542D"},x=["A meaning is a representation.","The word over the meaning.","The use is a state.","The word wants to use the meaning.","The meaning and the word perceive the state."],k="lira-sentence-reader-styles";class W{constructor(e){d(this,"container");d(this,"reading",!1);d(this,"requestToken",0);d(this,"documentResult");d(this,"expandedNodes",new Set);d(this,"selectedKey");d(this,"detailCache",new Map);d(this,"detailToken",0);this.client=e}mount(e){this.container=e,this.ensureStyles(),e.innerHTML=this.renderShell(),this.wire();const r=e.querySelector(".lira-sr-textarea");r!=null&&r.value&&this.read(r.value)}destroy(){this.container=void 0}wire(){const e=this.container;if(!e)return;const r=e.querySelector(".lira-sr-textarea"),t=e.querySelector(".lira-sr-read-btn");r&&t&&(t.addEventListener("click",()=>void this.read(r.value)),r.addEventListener("keydown",n=>{(n.metaKey||n.ctrlKey)&&n.key==="Enter"&&this.read(r.value)})),e.querySelectorAll(".lira-sr-example").forEach(n=>{n.addEventListener("click",()=>{const i=n.dataset.example??"";r&&(r.value=i),this.read(i)})});const s=e.querySelector(".lira-sr-tree");s==null||s.addEventListener("click",n=>this.handleTreeClick(n)),s==null||s.addEventListener("keydown",n=>this.handleTreeKeydown(n))}handleTreeClick(e){const r=e.target,t=r.closest('[data-action="toggle-node"]');if(t){this.toggleNode(t.dataset.node??"");return}const s=r.closest('.lira-tree-row[data-kind="sentence"]');s&&this.selectSentenceNode(s.dataset.node??"")}handleTreeKeydown(e){if(e.key!=="Enter"&&e.key!==" ")return;const r=e.target;if(r.closest('[data-action="toggle-node"]'))return;const t=r.closest('.lira-tree-row[data-kind="sentence"]');t&&(e.preventDefault(),this.selectSentenceNode(t.dataset.node??""))}toggleNode(e){if(!e||!this.documentResult)return;const r=!this.expandedNodes.has(e);if(r?this.expandedNodes.add(e):this.expandedNodes.delete(e),r&&$(e)&&!this.detailCache.has(e)){this.selectSentenceNode(e);return}this.renderTreeInPlace()}renderTreeInPlace(){var r;if(!this.documentResult)return;const e=(r=this.container)==null?void 0:r.querySelector(".lira-sr-tree");e&&(e.innerHTML=this.renderTree(this.documentResult))}async selectSentenceNode(e){const r=this.documentResult,t=$(e);if(!r||!t)return;const s=r.blocks[t.blockIndex];if(!s||s.blockKind!=="paragraph")return;const n=s.sentences[t.sentenceIndex];if(!n)return;this.selectedKey=e,this.expandedNodes.add(`b${t.blockIndex}`),this.expandedNodes.add(e),this.renderTreeInPlace();const i=this.detailCache.get(e);if(i){this.renderDetail(i);return}this.setPanelPlaceholder(".lira-sr-predicted","Loading…"),this.setPanelPlaceholder(".lira-sr-winner-panel","Loading…"),this.setPanelPlaceholder(".lira-sr-trace","Loading…");const o=++this.detailToken,p=this.isLearningEnabled();try{const u=await this.client.read(n.text,p,!0);if(o!==this.detailToken||!this.container)return;this.detailCache.set(e,u),this.renderTreeInPlace(),this.renderDetail(u)}catch(u){if(o!==this.detailToken||!this.container)return;this.setError(u instanceof Error?u.message:String(u))}}isLearningEnabled(){var r;const e=(r=this.container)==null?void 0:r.querySelector(".lira-sr-learning-toggle");return(e==null?void 0:e.checked)??!0}async read(e){const r=e.trim();if(!r||this.reading)return;const t=++this.requestToken,s=this.isLearningEnabled();this.reading=!0,this.setBusy(!0),this.setError(void 0);try{const n=await this.client.readDocument(r,s);if(t!==this.requestToken||!this.container)return;this.renderDocument(n)}catch(n){if(t!==this.requestToken||!this.container)return;this.setError(n instanceof Error?n.message:String(n))}finally{t===this.requestToken&&(this.reading=!1,this.setBusy(!1))}}setBusy(e){var t;const r=(t=this.container)==null?void 0:t.querySelector(".lira-sr-read-btn");r&&(r.disabled=e,r.textContent=e?"Reading…":"Read")}setLearningStatus(e){var s;const r=(s=this.container)==null?void 0:s.querySelector(".lira-sr-learning-status");if(!r)return;if(!e||!e.enabled){r.textContent="Learning off";return}const t=e.recordedThisRead>0?` (+${e.recordedThisRead})`:"";r.textContent=`Learning: ${e.totalObservations} observation${e.totalObservations===1?"":"s"}${t}`}setError(e){var t;const r=(t=this.container)==null?void 0:t.querySelector(".lira-sr-error");r&&(r.textContent=e??"",r.style.display=e?"block":"none")}renderDocument(e){this.documentResult=e.document,this.detailCache.clear(),this.expandedNodes=new Set(["doc"]),this.selectedKey=void 0,this.setLearningStatus(e.learning);const r=K(e.document);if(r){this.selectSentenceNode(r);return}this.renderTreeInPlace(),this.setPanelPlaceholder(".lira-sr-predicted","No sentences found in this text."),this.setPanelPlaceholder(".lira-sr-winner-panel","No sentences found in this text."),this.setPanelPlaceholder(".lira-sr-trace","No sentences found in this text.")}renderDetail(e){var n,i,o;const r=(n=this.container)==null?void 0:n.querySelector(".lira-sr-predicted"),t=(i=this.container)==null?void 0:i.querySelector(".lira-sr-winner-panel"),s=(o=this.container)==null?void 0:o.querySelector(".lira-sr-trace");r&&(r.innerHTML=this.renderPredicted(e.predicted,e.words)),t&&(t.innerHTML=this.renderWinner(e.predicted,e.trace)),s&&(s.innerHTML=this.renderTrace(e.trace))}setPanelPlaceholder(e,r){var s;const t=(s=this.container)==null?void 0:s.querySelector(e);t&&(t.innerHTML=`<div class="lira-sr-placeholder">${l(r)}</div>`)}ensureStyles(){if(document.getElementById(k))return;const e=document.createElement("style");e.id=k,e.textContent=ee,document.head.appendChild(e)}renderShell(){return`
      <div class="lira-sr">
        <div class="lira-sr-input-card">
          ...
          <div class="lira-sr-examples">
            ${x.map(e=>`<button type="button" class="lira-sr-example" data-example="${l(e)}">${l(e)}</button>`).join("")}
          </div>
        </div>
    ...
    `}renderTree(e){const r=this.expandedNodes.has("doc"),t=m[e.validation]??"#7A7A7A",s=e.blocks.filter(p=>p.blockKind==="paragraph").length,n=e.blocks.length-s,i=e.blocks.reduce((p,u)=>p+(u.blockKind==="paragraph"?u.errors.length:0),0),o=[`${s} paragraph${s===1?"":"s"}`,n?`${n} heading${n===1?"":"s"}`:"",i?`${i} error${i===1?"":"s"}`:""].filter(Boolean).join(", ");return`
      <ul class="lira-tree-root">
        <li class="lira-tree-node">
          <div class="lira-tree-row" data-node="doc" data-kind="document">
            ...
            ${r?"":`<span class="lira-tree-summary">${l(o)}</span>`}
          </div>
        ...
        <li class="lira-tree-node lira-tree-leaf">
          <div class="lira-tree-row" data-node="${t}" data-kind="heading">
            ...
            <span class="lira-tree-label">${l(f(e.text,48))}</span>
          </div>
        </li>`;const s=this.expandedNodes.has(t),n=m[e.validation]??"#7A7A7A",i=[`${e.sentences.length} sentence${e.sentences.length===1?"":"s"}`,e.errors.length?`${e.errors.length} error${e.errors.length===1?"":"s"}`:""].filter(Boolean).join(", ");return`
      <li class="lira-tree-node">
        ...
        ${s?`<ul>${e.sentences.map((o,p)=>this.renderSentenceNode(o,t,p)).join("")}</ul>`:""}
      </li>`}renderSentenceNode(e,r,t){const s=`${r}s${t}`,n=m[e.validation]??"#7A7A7A",i=this.expandedNodes.has(s),o=this.detailCache.get(s),p=i?`<ul>${o?o.predicted.clauses.map((u,B)=>this.renderClauseNode(u,`${s}c${B}`)).join(""):'<li class="lira-tree-node"><div class="lira-tree-row"><span class="lira-tree-spacer"></span><span class="lira-tree-summary">Loading…</span></div></li>'}</ul>`:"";return`
      <li class="lira-tree-node">
        <div class="lira-tree-row ${this.selectedKey===s?"selected":""}" data-node="${s}" data-kind="sentence" role="button" tabindex="0">
          ...
          <span class="lira-tree-snippet">${l(f(e.text,40))}</span>
          ${e.errors.length?`<span class="lira-tree-error-count">${e.errors.length}</span>`:""}
        </div>
        ${p}
      </li>`}renderClauseNode(e,r,t){const s=m[e.validation]??"#7A7A7A",i=[["subject",e.subject],["predicate",e.predicate],["object",e.object],["complement",e.complement],...e.modifiers.map(o=>["modifier",o])].filter(o=>o[1]!==null).map(([o,p],u)=>R(p)?this.renderClauseNode(p,`${r}r${u}`,o):this.renderPhraseNode(p,`${r}r${u}`,o)).join("");return`
      <li class="lira-tree-node">
        <div class="lira-tree-row" data-node="${r}" data-kind="clause">
          <span class="lira-tree-spacer"></span>
          <span class="lira-tree-dot" style="background:${s}"></span>
          ${t?`<span class="lira-tree-role-pill">${l(t)}</span>`:""}
          <span class="lira-tree-label">Clause</span>
          <span class="lira-tree-summary">${l(e.clauseType??"?")}</span>
          <span class="lira-tree-snippet">${l(f(e.text,40))}</span>
        </div>
        ${i?`<ul>${i}</ul>`:""}
      </li>`}renderPhraseNode(e,r,t){const s=m[e.validation]??"#7A7A7A",n=e.nestedPhrases.map((i,o)=>this.renderPhraseNode(i,`${r}n${o}`)).join("");return`
      <li class="lira-tree-node">
        <div class="lira-tree-row" data-node="${r}" data-kind="phrase">
          ...
          <span class="lira-tree-summary">${l(e.phraseType??"?")}</span>
          <span class="lira-tree-snippet">${l(f(e.text,40))}</span>
        </div>
        ${n?`<ul>${n}</ul>`:""}
      </li>`}renderWinner(e,r){const t=e.clauses[0],n=(t?[["subject",t.subject],["predicate",t.predicate],["object",t.object],["complement",t.complement],...t.modifiers.map(o=>["modifier",o])]:[]).map(([o,p])=>T(o,p)).join(""),i=r.length?`<div class="lira-sr-winner-positions">
          ${r.map(o=>`
            <span class="lira-sr-winner-chip">
              <span class="lira-sr-mono lira-sr-faint">#${o.startIndex}</span>
              <span class="lira-sr-strong lira-sr-mono">${l(o.tokenText??"")}</span>
              <span class="lira-sr-faint">${l(o.winnerPhraseType??"none")}</span>
            </span>`).join("")}
        </div>`:"";return`
    ...
    `}renderPredicted(e,r){return`${`
      ...
      <div class="lira-sr-attempt ${e.startMatch?"":"rejected"}">
        ...
        ${r}
      </div>`}}function R(a){return"clauseType"in a}function T(a,e){if(!e)return"";if(R(e)){const r=[["subject",e.subject],["predicate",e.predicate],["object",e.object],["complement",e.complement],...e.modifiers.map(t=>["modifier",t])];return`
      <div class="lira-sr-winner-role-row">
        <span class="lira-sr-winner-role-label">${l(a)}</span>
        <span class="lira-sr-mono">"${l(e.text)}"</span>
        <span class="lira-sr-faint">embedded clause · ${l(e.clauseType??"?")}</span>
      </div>
      <div class="lira-sr-winner-roles lira-sr-winner-roles-nested">${r.map(([t,s])=>T(t,s)).join("")}</div>`}return`
    <div class="lira-sr-winner-role-row">
      <span class="lira-sr-winner-role-label">${l(a)}</span>
      <span class="lira-sr-mono">"${l(e.text)}"</span>
      <span class="lira-sr-faint">${l(e.phraseType??"?")}</span>
    </div>`}function b(a,e){return`<button type="button" class="lira-tree-toggle" data-action="toggle-node" data-node="${a}" aria-expanded="${e}">${Z}</button>`}function f(a,e){const r=a.trim();return r.length>e?`${r.slice(0,e-1)}…`:r}function $(a){const e=/^b(\d+)s(\d+)$/.exec(a);if(e)return{blockIndex:Number(e[1]),sentenceIndex:Number(e[2])}}function K(a){for(let e=0;e<a.blocks.length;e+=1){const r=a.blocks[e];if(r.blockKind!=="paragraph")continue;const t=r.sentences.findIndex(s=>s.errors.length>0);if(t>=0)return`b${e}s${t}`}for(let e=0;e<a.blocks.length;e+=1){const r=a.blocks[e];if(r.blockKind==="paragraph"&&r.sentences.length>0)return`b${e}s0`}}function Y(a){if(!a.length)return'<div class="lira-sr-empty">No words to show.</div>';let e="";return a.forEach((r,t)=>{const s=/^[.,!?;:]+$/.test(r.text);t>0&&!s&&(e+=" "),e+=J(r,t)}),`<div class="lira-sr-sentence">${e}</div>`}function J(a,e){return`<span class="lira-sr-word${a.resolved?"":" lira-sr-word-unfound"}" tabindex="0" data-word-index="${e}">${l(a.text)}${G(a)}</span>`}function G(a){if(!a.resolved)return`<span class="lira-sr-word-tooltip"><span class="tt-title">${l(a.text)}</span><span class="tt-meta">Not found in the Common Vocabulary Cache</span></span>`;const e=[a.validation??"UNRESOLVED",a.phraseType??"no phrase"],r=[a.partOfSpeech??"?",`conf ${a.confidence!==null?a.confidence.toFixed(2):"—"}`];return`<span class="lira-sr-word-tooltip"><span class="tt-title">${l(a.text)}</span><span class="tt-meta">${e.map(l).join(" · ")}</span><span class="tt-meta">${r.map(l).join(" · ")}</span></span>`}function E(a){return`<span class="lira-sr-badge" style="background:${m[a]??"#7A7A7A"}">${l(a)}</span>`}function L(a,e){return`
    <span class="lira-sr-pos-chip" style="background:${_[e]??"#7A7A7A"}">
      <span class="w">${l(a)}</span>
      <span class="p">${l(e)}</span>
    </span>`}function X(a){return a.length?`<div class="lira-sr-trace-tokens">${a.map(r=>L(r.text,r.partOfSpeech??"UNKNOWN")).join("")}</div>`:""}function Q(a){return a.length?`
    ...
    </div>`:""}function l(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const Z='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6l4 4 4-4"/></svg>',ee=`
...
${t.script}
      ...
      <div class="portal-mode-toggle" role="group" aria-label="Layout">
        <button type="button" data-action="mode" data-mode="desktop" class="${this.mode==="desktop"?"active":""}">${oe} Desktop</button>
        <button type="button" data-action="mode" data-mode="mobile" class="${this.mode==="mobile"?"active":""}">${le} Mobile</button>
      </div>`;if(this.mode==="mobile")return this.mobileScreen==="view"&&e?`
          <div class="portal-topbar">
            <button type="button" class="portal-back" data-action="back" aria-label="Back to Domains">${ie}</button>
            <span class="portal-topbar-title">${h(e.name)}</span>
            ${r}
          </div>`:`<div class="portal-topbar"><span class="portal-topbar-title">All Domains</span>${r}</div>`;const t=e?this.registry.ancestryOf(e.name):[];return`<div class="portal-topbar"><nav class="portal-breadcrumb">${[`<span class="crumb-root">${ne} All Domains</span>`,...t.map(n=>`<span>${h(n.name)}</span>`)].join(`<span class="crumb-sep">${ae}</span>`)}</nav>${r}</div>`}renderBody(e){return this.mode==="desktop"?`${this.renderTree()}${this.renderViewPane(e)}`:this.mobileScreen==="browse"?`<div class="portal-tree portal-tree--mobile">${this.renderTreeRows(0)}</div>`:this.renderViewPane(e,!0)}renderTree(){return this.treeCollapsed?`
        <nav class="portal-tree portal-tree--collapsed">
          <button type="button" class="portal-tree-toggle" data-action="toggle-tree" title="Expand Domains" aria-label="Expand Domains" aria-expanded="false">${ce}</button>
        </nav>`:`
      <nav class="portal-tree">
        <div class="portal-tree-label">
          <span>Domains</span>
          <button type="button" class="portal-tree-toggle" data-action="toggle-tree" title="Collapse Domains" aria-label="Collapse Domains" aria-expanded="true">${de}</button>
        </div>
        ${this.renderTreeRows(0)}
          ...
          <div class="portal-tree-row depth-${e} ${i?"selected":""}" data-action="select" data-domain="${h(s.name)}">
            ${n.length>0?se:'<span class="chev-spacer"></span>'}
            ${te}
      ...
      <div class="portal-component-switcher" role="tablist" aria-label="UI Component">
        ...
        <div class="portal-view ${r?"portal-view--full":""}">
          ...
          ${s}
        </div>`;const i=S.find(p=>p.id===this.selectedComponent),o=i!=null&&i.available?`
      ...
      <div class="portal-view ${r?"portal-view--full":""}">
        ...
        ${o}
        ${s}
      </div>`}}function h(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const te='<svg class="i-folder" viewBox="0 0 16 16" fill="currentColor"><path d="M1.5 3.5A1 1 0 0 1 2.5 2.5h3.6l1.2 1.4H13.5A1 1 0 0 1 14.5 5v7A1 1 0 0 1 13.5 13h-11a1 1 0 0 1-1-1v-8.5z"/></svg>',se='<svg class="i-chev" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6l4 4 4-4"/></svg>',ae='<svg class="i-chev-right" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 3l5 5-5 5"/></svg>',ne='<svg class="i-home" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M2 8l6-5 6 5M4 7v6h8V7"/></svg>',ie='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 3L5 8l5 5"/></svg>',oe='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="1.5" y="2.5" width="13" height="9" rx="1"/><path d="M6 13.5h4"/></svg>',le='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="4.5" y="1.5" width="7" height="13" rx="1.4"/></svg>',de='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 3L5 8l5 5"/></svg>',ce='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 3l5 5-5 5"/></svg>',pe=`
.portal-shell {
