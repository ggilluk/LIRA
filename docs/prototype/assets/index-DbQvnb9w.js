var T=Object.defineProperty;var I=(a,e,r)=>e in a?T(a,e,{enumerable:!0,configurable:!0,writable:!0,value:r}):a[e]=r;var o=(a,e,r)=>I(a,typeof e!="symbol"?e+"":e,r);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))t(n);new MutationObserver(n=>{for(const s of n)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&t(i)}).observe(document,{childList:!0,subtree:!0});function r(n){const s={};return n.integrity&&(s.integrity=n.integrity),n.referrerPolicy&&(s.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?s.credentials="include":n.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function t(n){if(n.ep)return;n.ep=!0;const s=r(n);fetch(n.href,s)}})();class L{constructor(){o(this,"statuses",new Map);o(this,"listeners",new Set)}register(e,r,t="idle",n){this.statuses.set(e,{id:e,label:r,state:t,detail:n}),this.notify()}update(e,r,t,n){const s=this.statuses.get(e);s&&(this.statuses.set(e,{...s,state:r,detail:t,progress:n}),this.notify())}get(e){return this.statuses.get(e)}all(){return[...this.statuses.values()]}subscribe(e){return this.listeners.add(e),e(this.all()),()=>{this.listeners.delete(e)}}notify(){const e=this.all();for(const r of this.listeners)r(e)}}class B{constructor(e=[]){o(this,"domains",new Map);for(const r of e)this.add(r)}add(e){this.domains.set(e.name,e)}get(e){return this.domains.get(e)}all(){return[...this.domains.values()]}roots(){return this.all().filter(e=>e.parentName===void 0)}children(e){return this.all().filter(r=>r.parentName===e)}ancestryOf(e){const r=[];let t=this.get(e);for(;t!==void 0;)r.unshift(t),t=t.parentName!==void 0?this.get(t.parentName):void 0;return r}}class A{constructor(e,r="LIRA"){o(this,"unsubscribe");this.board=e,this.title=r}mount(e){var r;this.ensureStyles(),(r=this.unsubscribe)==null||r.call(this),this.unsubscribe=this.board.subscribe(t=>{e.innerHTML=this.renderScreen(t)})}waitFor(...e){return new Promise(r=>{const t=i=>i==="done"||i==="error",n=i=>{const c=i.filter(p=>e.includes(p.id));c.length===e.length&&c.every(p=>t(p.state))&&(s(),r())},s=this.board.subscribe(n)})}destroy(){var e;(e=this.unsubscribe)==null||e.call(this),this.unsubscribe=void 0}renderScreen(e){return`
    ...
    `}ensureStyles(){if(document.getElementById(v))return;const e=document.createElement("style");e.id=v,e.textContent=O,document.head.appendChild(e)}}const D={"not-ported":"Not ported yet",idle:"Waiting…",running:"Working…",done:"Ready",error:"Failed"},M={"not-ported":"–",idle:"○",running:'<span class="loading-spinner"></span>',done:"✓",error:"✕"};function f(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const v="lira-loading-screen-styles",O=`
.loading-screen {
    ...
    `}renderRow(e){const r=this.actions.find(n=>n.id===e.id),t=e.progress!==void 0?`<div class="service-status-progress"><div class="service-status-progress-fill" style="width:${Math.round(e.progress*100)}%"></div></div>`:"";return`
      <div class="service-status-row-group">
        <div class="service-status-row state-${e.state}">
          ...
          <span class="service-status-name">${h(e.label)}</span>
          <span class="service-status-pill">${V[e.state]}</span>
          ${e.detail?`<span class="service-status-detail">${h(e.detail)}</span>`:""}
          ${r?`<button type="button" class="service-status-action" data-action="run" data-action-id="${h(r.id)}" ${e.state==="running"?"disabled":""}>${h(r.label)}</button>`:""}
        </div>
        ${t}
      </div>
    `}ensureStyles(){if(document.getElementById(b))return;const e=document.createElement("style");e.id=b,e.textContent=P,document.head.appendChild(e)}}const V={"not-ported":"Not ported",idle:"Idle",running:"Running",done:"Running",error:"Error"};function h(a){return a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const z='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6l4 4 4-4"/></svg>',b="lira-service-status-styles",P=`
...
`,E="Common",C=[{id:"vocabulary",label:"Vocabulary",available:!0},{id:"linguistics",label:"Linguistics",available:!0},{id:"knowledge",label:"Knowledge",available:!1}],S="lira-portal-shell-styles",R="lira-vocabulary-fragment-styles";class Z{constructor(e,r,t,n,s={}){o(this,"mode");o(this,"mobileScreen","browse");o(this,"selectedName");o(this,"selectedComponent","vocabulary");o(this,"treeCollapsed",!1);o(this,"title");o(this,"container");o(this,"serviceStatusView");o(this,"sentenceReaderView");o(this,"renderToken",0);o(this,"currentVocabularyDomainName");var i;this.registry=e,this.vocabularyClient=r,this.statusBoard=n,this.title=s.title??"LIRA",this.mode=typeof window<"u"&&window.matchMedia("(max-width: 720px)").matches?"mobile":"desktop",this.selectedName=(i=this.registry.roots()[0])==null?void 0:i.name,this.serviceStatusView=new F(n),this.sentenceReaderView=new U(t),this.statusBoard.subscribe(c=>{const p=c.find(u=>u.id==="vocabulary");p&&this.updateVocabToolbar(p)}),this.vocabularyClient.onDomainUpdated(c=>{this.registry.add(c),this.render()}),this.searchWordsBridge(),this.searchRelationshipsBridge()}searchWordsBridge(){document.addEventListener("lira-search-words",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.searchWords(this.currentVocabularyDomainName,{word:r.word,gloss:r.gloss,definition:r.definition,pos:r.pos,domainLabel:r.domain,rootWordsOnly:r.rootWordsOnly,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-search-words-result",{detail:{requestId:r.requestId,words:t.words,totalMatches:t.totalMatches}}))})})}searchRelationshipsBridge(){document.addEventListener("lira-search-relationships",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.searchRelationships(this.currentVocabularyDomainName,{wordId:r.wordId,query:r.query,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-search-relationships-result",{detail:{requestId:r.requestId,relationships:t.relationships,totalMatches:t.totalMatches}}))})})}mount(e){this.container=e,document.title=this.title,this.ensureStyles(),e.addEventListener("click",r=>this.handleClick(r)),this.render()}handleClick(e){const r=e.target.closest("[data-action]");if(!r||r.disabled)return;const t=r.dataset.action;t==="select"?(this.selectedName=r.dataset.domain,this.mode==="mobile"&&(this.mobileScreen="view"),this.render()):t==="mode"?(this.mode=r.dataset.mode,this.mode==="mobile"&&(this.mobileScreen=this.selectedName?"view":"browse"),this.render()):t==="back"?(this.mobileScreen="browse",this.render()):t==="component"?(this.selectedComponent=r.dataset.component,this.render()):t==="toggle-tree"?(this.treeCollapsed=!this.treeCollapsed,this.render()):t==="seed-wordnet"?this.vocabularyClient.seedWordNet(E):t==="seed-common-vocabulary"&&this.vocabularyClient.seedCommonVocabulary(E)}renderVocabToolbar(){return`<div class="portal-vocab-toolbar">${this.vocabToolbarInner(this.statusBoard.get("vocabulary"))}</div>`}vocabToolbarInner(e){const r=(e==null?void 0:e.state)==="running",t=e==null?void 0:e.progress;return`
      ...
      <span class="portal-vocab-toolbar-detail">${e!=null&&e.detail?m(e.detail):""}</span>
...
${t.script}
      ...
      <div class="portal-mode-toggle" role="group" aria-label="Layout">
        ...
        <button type="button" data-action="mode" data-mode="mobile" class="${this.mode==="mobile"?"active":""}">${ie} Mobile</button>
          ...
          <div class="portal-topbar">
            ...
            ${r}
          </div>`:`<div class="portal-topbar"><span class="portal-topbar-title">All Domains</span>${r}</div>`;const t=e?this.registry.ancestryOf(e.name):[];return`<div class="portal-topbar"><nav class="portal-breadcrumb">${[`<span class="crumb-root">${ne} All Domains</span>`,...t.map(s=>`<span>${m(s.name)}</span>`)].join(`<span class="crumb-sep">${te}</span>`)}</nav>${r}</div>`}renderBody(e){return this.mode==="desktop"?`${this.renderTree()}${this.renderViewPane(e)}`:this.mobileScreen==="browse"?`<div class="portal-tree portal-tree--mobile">${this.renderTreeRows(0)}</div>`:this.renderViewPane(e,!0)}renderTree(){return this.treeCollapsed?`
      ...
      <nav class="portal-tree">
        ...
        ${this.renderTreeRows(0)}
          ...
          <div class="portal-tree-row depth-${e} ${i?"selected":""}" data-action="select" data-domain="${m(n.name)}">
            ${s.length>0?re:'<span class="chev-spacer"></span>'}
            ${ee}
            <span class="name">${m(n.name)}</span>
