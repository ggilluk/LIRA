var L=Object.defineProperty;var T=(n,e,r)=>e in n?L(n,e,{enumerable:!0,configurable:!0,writable:!0,value:r}):n[e]=r;var o=(n,e,r)=>T(n,typeof e!="symbol"?e+"":e,r);(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))t(s);new MutationObserver(s=>{for(const i of s)if(i.type==="childList")for(const a of i.addedNodes)a.tagName==="LINK"&&a.rel==="modulepreload"&&t(a)}).observe(document,{childList:!0,subtree:!0});function r(s){const i={};return s.integrity&&(i.integrity=s.integrity),s.referrerPolicy&&(i.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?i.credentials="include":s.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function t(s){if(s.ep)return;s.ep=!0;const i=r(s);fetch(s.href,i)}})();class B{constructor(){o(this,"statuses",new Map);o(this,"listeners",new Set)}register(e,r,t="idle",s){this.statuses.set(e,{id:e,label:r,state:t,detail:s}),this.notify()}update(e,r,t,s){const i=this.statuses.get(e);i&&(this.statuses.set(e,{...i,state:r,detail:t,progress:s}),this.notify())}get(e){return this.statuses.get(e)}all(){return[...this.statuses.values()]}subscribe(e){return this.listeners.add(e),e(this.all()),()=>{this.listeners.delete(e)}}notify(){const e=this.all();for(const r of this.listeners)r(e)}}class M{constructor(e=[]){o(this,"domains",new Map);for(const r of e)this.add(r)}add(e){this.domains.set(e.name,e)}get(e){return this.domains.get(e)}all(){return[...this.domains.values()]}roots(){return this.all().filter(e=>e.parentName===void 0)}children(e){return this.all().filter(r=>r.parentName===e)}ancestryOf(e){const r=[];let t=this.get(e);for(;t!==void 0;)r.unshift(t),t=t.parentName!==void 0?this.get(t.parentName):void 0;return r}}class D{constructor(e,r="LIRA"){o(this,"unsubscribe");this.board=e,this.title=r}mount(e){var r;this.ensureStyles(),(r=this.unsubscribe)==null||r.call(this),this.unsubscribe=this.board.subscribe(t=>{e.innerHTML=this.renderScreen(t)})}waitFor(...e){return new Promise(r=>{const t=a=>a==="done"||a==="error",s=a=>{const d=a.filter(p=>e.includes(p.id));d.length===e.length&&d.every(p=>t(p.state))&&(i(),r())},i=this.board.subscribe(s)})}destroy(){var e;(e=this.unsubscribe)==null||e.call(this),this.unsubscribe=void 0}renderScreen(e){return`
    ...
    `}renderStep(e){return`
      <div class="loading-step state-${e.state}">
        <span class="loading-step-icon">${V[e.state]}</span>
        <span class="loading-step-label">${f(e.label)}</span>
        <span class="loading-step-detail">${f(e.detail??A[e.state])}</span>
      </div>
    `}ensureStyles(){if(document.getElementById(v))return;const e=document.createElement("style");e.id=v,e.textContent=P,document.head.appendChild(e)}}const A={"not-ported":"Not ported yet",idle:"Waiting…",running:"Working…",done:"Ready",error:"Failed"},V={"not-ported":"–",idle:"○",running:'<span class="loading-spinner"></span>',done:"✓",error:"✕"};function f(n){return n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const v="lira-loading-screen-styles",P=`
.loading-screen {
...
`;class O{constructor(e,r=[]){o(this,"unsubscribe");o(this,"collapsed",!1);this.board=e,this.actions=r}mount(e){var r;this.ensureStyles(),(r=this.unsubscribe)==null||r.call(this),e.addEventListener("click",t=>this.handleClick(t,e)),this.unsubscribe=this.board.subscribe(t=>{e.innerHTML=this.renderPanel(t)})}destroy(){var e;(e=this.unsubscribe)==null||e.call(this),this.unsubscribe=void 0}handleClick(e,r){const t=e.target.closest("[data-action]");if(!(!t||t.disabled)){if(t.dataset.action==="toggle")this.collapsed=!this.collapsed,r.innerHTML=this.renderPanel(this.board.all());else if(t.dataset.action==="run"){const s=this.actions.find(i=>i.id===t.dataset.actionId);s==null||s.onClick()}}}renderPanel(e){const r=e.filter(t=>t.state==="running"||t.state==="done").length;return`
      <div class="service-status-panel ${this.collapsed?"collapsed":""}">
        <button type="button" class="service-status-header" data-action="toggle" aria-expanded="${!this.collapsed}">
          ...
          <span class="service-status-chevron">${q}</span>
        </button>
    ...
    `}renderRow(e){const r=this.actions.find(s=>s.id===e.id),t=e.progress!==void 0?`<div class="service-status-progress"><div class="service-status-progress-fill" style="width:${Math.round(e.progress*100)}%"></div></div>`:"";return`
      <div class="service-status-row-group">
        <div class="service-status-row state-${e.state}">
          ...
          <span class="service-status-pill">${F[e.state]}</span>
    ...
    `}ensureStyles(){if(document.getElementById(b))return;const e=document.createElement("style");e.id=b,e.textContent=z,document.head.appendChild(e)}}const F={"not-ported":"Not ported",idle:"Idle",running:"Running",done:"Running",error:"Error"};function m(n){return n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}const q='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6l4 4 4-4"/></svg>',b="lira-service-status-styles",z=`
...
`;var c=(n=>(n[n.NOUN=0]="NOUN",n[n.VERB=1]="VERB",n[n.ADJECTIVE=2]="ADJECTIVE",n[n.ADVERB=3]="ADVERB",n[n.PRONOUN=4]="PRONOUN",n[n.DETERMINER=5]="DETERMINER",n[n.PREPOSITION=6]="PREPOSITION",n[n.CONJUNCTION=7]="CONJUNCTION",n[n.INTERJECTION=8]="INTERJECTION",n[n.NUMERAL=9]="NUMERAL",n[n.AUXILIARY=11]="AUXILIARY",n[n.PROPER_NOUN=12]="PROPER_NOUN",n[n.SYMBOL=13]="SYMBOL",n[n.PUNCTUATION=14]="PUNCTUATION",n[n.OTHER=15]="OTHER",n))(c||{});const j={[c[c.NOUN]]:"#3B6EA5",[c[c.PROPER_NOUN]]:"#274472",[c[c.VERB]]:"#B2542D",[c[c.ADJECTIVE]]:"#7A5CA6",[c[c.ADVERB]]:"#B08900",[c[c.PRONOUN]]:"#5B7B6F",[c[c.DETERMINER]]:"#6E7B8B",[c[c.PREPOSITION]]:"#7B6E5B",[c[c.CONJUNCTION]]:"#6B7280",[c[c.AUXILIARY]]:"#5B6E8B",[c[c.INTERJECTION]]:"#C2544B",[c[c.NUMERAL]]:"#4B8A7B",[c[c.SYMBOL]]:"#8A8A8A",[c[c.PUNCTUATION]]:"#9A9A9A",[c[c.OTHER]]:"#7A7A7A"},g={VALID:"#2B6E63",UNRESOLVED:"#B08900",INVALID:"#B2542D"},w=["A meaning is a representation.","The word over the meaning.","The use is a state.","The word wants to use the meaning.","The meaning and the word perceive the state."],y="lira-sentence-reader-styles";class U{constructor(e){o(this,"container");o(this,"reading",!1);o(this,"requestToken",0);o(this,"documentResult");o(this,"expandedNodes",new Set);o(this,"selectedKey");o(this,"detailCache",new Map);o(this,"detailToken",0);this.client=e}mount(e){this.container=e,this.ensureStyles(),e.innerHTML=this.renderShell(),this.wire();const r=e.querySelector(".lira-sr-textarea");r!=null&&r.value&&this.read(r.value)}destroy(){this.container=void 0}wire(){const e=this.container;if(!e)return;const r=e.querySelector(".lira-sr-textarea"),t=e.querySelector(".lira-sr-read-btn");r&&t&&(t.addEventListener("click",()=>void this.read(r.value)),r.addEventListener("keydown",i=>{(i.metaKey||i.ctrlKey)&&i.key==="Enter"&&this.read(r.value)})),e.querySelectorAll(".lira-sr-example").forEach(i=>{i.addEventListener("click",()=>{const a=i.dataset.example??"";r&&(r.value=a),this.read(a)})});const s=e.querySelector(".lira-sr-tree");s==null||s.addEventListener("click",i=>this.handleTreeClick(i)),s==null||s.addEventListener("keydown",i=>this.handleTreeKeydown(i))}handleTreeClick(e){const r=e.target,t=r.closest('[data-action="toggle-node"]');if(t){this.toggleNode(t.dataset.node??"");return}const s=r.closest('.lira-tree-row[data-kind="sentence"]');s&&this.selectSentenceNode(s.dataset.node??"")}handleTreeKeydown(e){if(e.key!=="Enter"&&e.key!==" ")return;const r=e.target.closest('.lira-tree-row[data-kind="sentence"]');r&&(e.preventDefault(),this.selectSentenceNode(r.dataset.node??""))}toggleNode(e){!e||!this.documentResult||(this.expandedNodes.has(e)?this.expandedNodes.delete(e):this.expandedNodes.add(e),this.renderTreeInPlace())}renderTreeInPlace(){var r;if(!this.documentResult)return;const e=(r=this.container)==null?void 0:r.querySelector(".lira-sr-tree");e&&(e.innerHTML=this.renderTree(this.documentResult))}async selectSentenceNode(e){const r=this.documentResult,t=_(e);if(!r||!t)return;const s=r.blocks[t.blockIndex];if(!s||s.blockKind!=="paragraph")return;const i=s.sentences[t.sentenceIndex];if(!i)return;this.selectedKey=e,this.expandedNodes.add(`b${t.blockIndex}`),this.renderTreeInPlace();const a=this.detailCache.get(e);if(a){this.renderDetail(a);return}this.setPanelPlaceholder(".lira-sr-predicted","Loading…"),this.setPanelPlaceholder(".lira-sr-winner-panel","Loading…"),this.setPanelPlaceholder(".lira-sr-trace","Loading…");const d=++this.detailToken,p=this.isLearningEnabled();try{const u=await this.client.read(i.text,p,!0);if(d!==this.detailToken||!this.container)return;this.detailCache.set(e,u),this.renderDetail(u)}catch(u){if(d!==this.detailToken||!this.container)return;this.setError(u instanceof Error?u.message:String(u))}}isLearningEnabled(){var r;const e=(r=this.container)==null?void 0:r.querySelector(".lira-sr-learning-toggle");return(e==null?void 0:e.checked)??!0}async read(e){const r=e.trim();if(!r||this.reading)return;const t=++this.requestToken,s=this.isLearningEnabled();this.reading=!0,this.setBusy(!0),this.setError(void 0);try{const i=await this.client.readDocument(r,s);if(t!==this.requestToken||!this.container)return;this.renderDocument(i)}catch(i){if(t!==this.requestToken||!this.container)return;this.setError(i instanceof Error?i.message:String(i))}finally{t===this.requestToken&&(this.reading=!1,this.setBusy(!1))}}setBusy(e){var t;const r=(t=this.container)==null?void 0:t.querySelector(".lira-sr-read-btn");r&&(r.disabled=e,r.textContent=e?"Reading…":"Read")}setLearningStatus(e){var s;const r=(s=this.container)==null?void 0:s.querySelector(".lira-sr-learning-status");if(!r)return;if(!e||!e.enabled){r.textContent="Learning off";return}const t=e.recordedThisRead>0?` (+${e.recordedThisRead})`:"";r.textContent=`Learning: ${e.totalObservations} observation${e.totalObservations===1?"":"s"}${t}`}setError(e){var t;const r=(t=this.container)==null?void 0:t.querySelector(".lira-sr-error");r&&(r.textContent=e??"",r.style.display=e?"block":"none")}renderDocument(e){this.documentResult=e.document,this.detailCache.clear(),this.expandedNodes=new Set(["doc"]),this.selectedKey=void 0,this.setLearningStatus(e.learning);const r=W(e.document);if(r){this.selectSentenceNode(r);return}this.renderTreeInPlace(),this.setPanelPlaceholder(".lira-sr-predicted","No sentences found in this text."),this.setPanelPlaceholder(".lira-sr-winner-panel","No sentences found in this text."),this.setPanelPlaceholder(".lira-sr-trace","No sentences found in this text.")}renderDetail(e){var i,a,d;const r=(i=this.container)==null?void 0:i.querySelector(".lira-sr-predicted"),t=(a=this.container)==null?void 0:a.querySelector(".lira-sr-winner-panel"),s=(d=this.container)==null?void 0:d.querySelector(".lira-sr-trace");r&&(r.innerHTML=this.renderPredicted(e.predicted,e.words)),t&&(t.innerHTML=this.renderWinner(e.predicted,e.trace)),s&&(s.innerHTML=this.renderTrace(e.trace))}setPanelPlaceholder(e,r){var s;const t=(s=this.container)==null?void 0:s.querySelector(e);t&&(t.innerHTML=`<div class="lira-sr-placeholder">${l(r)}</div>`)}ensureStyles(){if(document.getElementById(y))return;const e=document.createElement("style");e.id=y,e.textContent=Z,document.head.appendChild(e)}renderShell(){return`
      <div class="lira-sr">
    ...
    `}renderTree(e){const r=this.expandedNodes.has("doc"),t=g[e.validation]??"#7A7A7A",s=e.blocks.filter(p=>p.blockKind==="paragraph").length,i=e.blocks.length-s,a=e.blocks.reduce((p,u)=>p+(u.blockKind==="paragraph"?u.errors.length:0),0),d=[`${s} paragraph${s===1?"":"s"}`,i?`${i} heading${i===1?"":"s"}`:"",a?`${a} error${a===1?"":"s"}`:""].filter(Boolean).join(", ");return`
      <ul class="lira-tree-root">
        <li class="lira-tree-node">
          <div class="lira-tree-row" data-node="doc" data-kind="document">
            ...
            ${r?"":`<span class="lira-tree-summary">${l(d)}</span>`}
          </div>
        ...
        </li>`;const s=this.expandedNodes.has(t),i=g[e.validation]??"#7A7A7A",a=[`${e.sentences.length} sentence${e.sentences.length===1?"":"s"}`,e.errors.length?`${e.errors.length} error${e.errors.length===1?"":"s"}`:""].filter(Boolean).join(", ");return`
      <li class="lira-tree-node">
        <div class="lira-tree-row" data-node="${t}" data-kind="paragraph">
          ...
          ${s?"":`<span class="lira-tree-summary">${l(a)}</span>`}
        </div>
        ${s?`<ul>${e.sentences.map((d,p)=>this.renderSentenceNode(d,t,p)).join("")}</ul>`:""}
      </li>`}renderSentenceNode(e,r,t){const s=`${r}s${t}`,i=g[e.validation]??"#7A7A7A";return`
      <li class="lira-tree-node lira-tree-leaf">
      ...
      </li>`}renderWinner(e,r){const t=e.clauses[0],i=(t?[["subject",t.subject],["predicate",t.predicate],["object",t.object],["complement",t.complement],...t.modifiers.map(d=>["modifier",d])]:[]).map(([d,p])=>R(d,p)).join(""),a=r.length?`<div class="lira-sr-winner-positions">
          ${r.map(d=>`
            <span class="lira-sr-winner-chip">
              <span class="lira-sr-mono lira-sr-faint">#${d.startIndex}</span>
              <span class="lira-sr-strong lira-sr-mono">${l(d.tokenText??"")}</span>
              <span class="lira-sr-faint">${l(d.winnerPhraseType??"none")}</span>
            </span>`).join("")}
        </div>`:"";return`
    ...
    `}renderPredicted(e,r){return`${`
      ...
      <div class="lira-sr-attempt ${e.startMatch?"":"rejected"}">
        ...
        ${r}
      </div>`}}function H(n){return"clauseType"in n}function R(n,e){if(!e)return"";if(H(e)){const r=[["subject",e.subject],["predicate",e.predicate],["object",e.object],["complement",e.complement],...e.modifiers.map(t=>["modifier",t])];return`
      ...
      <div class="lira-sr-winner-roles lira-sr-winner-roles-nested">${r.map(([t,s])=>R(t,s)).join("")}</div>`}return`
    <div class="lira-sr-winner-role-row">
      <span class="lira-sr-winner-role-label">${l(n)}</span>
      <span class="lira-sr-mono">"${l(e.text)}"</span>
      <span class="lira-sr-faint">${l(e.phraseType??"?")}</span>
    </div>`}function x(n,e){return`<button type="button" class="lira-tree-toggle" data-action="toggle-node" data-node="${n}" aria-expanded="${e}">${Q}</button>`}function k(n,e){const r=n.trim();return r.length>e?`${r.slice(0,e-1)}…`:r}function _(n){const e=/^b(\d+)s(\d+)$/.exec(n);if(e)return{blockIndex:Number(e[1]),sentenceIndex:Number(e[2])}}function W(n){for(let e=0;e<n.blocks.length;e+=1){const r=n.blocks[e];if(r.blockKind!=="paragraph")continue;const t=r.sentences.findIndex(s=>s.errors.length>0);if(t>=0)return`b${e}s${t}`}for(let e=0;e<n.blocks.length;e+=1){const r=n.blocks[e];if(r.blockKind==="paragraph"&&r.sentences.length>0)return`b${e}s0`}}function K(n){if(!n.length)return'<div class="lira-sr-empty">No words to show.</div>';let e="";return n.forEach((r,t)=>{const s=/^[.,!?;:]+$/.test(r.text);t>0&&!s&&(e+=" "),e+=Y(r,t)}),`<div class="lira-sr-sentence">${e}</div>`}function Y(n,e){return`<span class="lira-sr-word${n.resolved?"":" lira-sr-word-unfound"}" tabindex="0" data-word-index="${e}">${l(n.text)}${J(n)}</span>`}function J(n){if(!n.resolved)return`<span class="lira-sr-word-tooltip"><span class="tt-title">${l(n.text)}</span><span class="tt-meta">Not found in the Common Vocabulary Cache</span></span>`;const e=[n.validation??"UNRESOLVED",n.phraseType??"no phrase"],r=[n.partOfSpeech??"?",`conf ${n.confidence!==null?n.confidence.toFixed(2):"—"}`];return`<span class="lira-sr-word-tooltip"><span class="tt-title">${l(n.text)}</span><span class="tt-meta">${e.map(l).join(" · ")}</span><span class="tt-meta">${r.map(l).join(" · ")}</span></span>`}function $(n){return`<span class="lira-sr-badge" style="background:${g[n]??"#7A7A7A"}">${l(n)}</span>`}function N(n,e){return`
    <span class="lira-sr-pos-chip" style="background:${j[e]??"#7A7A7A"}">
      <span class="w">${l(n)}</span>
      <span class="p">${l(e)}</span>
    </span>`}function G(n){return n.length?`<div class="lira-sr-trace-tokens">${n.map(r=>N(r.text,r.partOfSpeech??(r.isMarker?"MARKER":"UNKNOWN"))).join("")}</div>`:""}function X(n){return n.length?`
    ...
    </div>`:""}function l(n){return n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const Q='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6l4 4 4-4"/></svg>',Z=`
...
`,E="Common",C=[{id:"vocabulary",label:"Vocabulary",available:!0},{id:"linguistics",label:"Linguistics",available:!0},{id:"knowledge",label:"Knowledge",available:!1}],S="lira-portal-shell-styles",I="lira-vocabulary-fragment-styles";class ee{constructor(e,r,t,s,i={}){o(this,"mode");o(this,"mobileScreen","browse");o(this,"selectedName");o(this,"selectedComponent","vocabulary");o(this,"treeCollapsed",!1);o(this,"title");o(this,"container");o(this,"serviceStatusView");o(this,"sentenceReaderView");o(this,"renderToken",0);o(this,"currentVocabularyDomainName");var a;this.registry=e,this.vocabularyClient=r,this.statusBoard=s,this.title=i.title??"LIRA",this.mode=typeof window<"u"&&window.matchMedia("(max-width: 720px)").matches?"mobile":"desktop",this.selectedName=(a=this.registry.roots()[0])==null?void 0:a.name,this.serviceStatusView=new O(s),this.sentenceReaderView=new U(t),this.statusBoard.subscribe(d=>{const p=d.find(u=>u.id==="vocabulary");p&&this.updateVocabToolbar(p)}),this.vocabularyClient.onDomainUpdated(d=>{this.registry.add(d),this.render()}),this.searchWordsBridge(),this.searchPhrasesBridge(),this.searchSensesBridge(),this.searchRelationshipsBridge(),this.searchLexicalRelationshipsBridge(),this.resolveHierarchyBridge()}searchWordsBridge(){document.addEventListener("lira-search-words",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.searchWords(this.currentVocabularyDomainName,{wordId:r.wordId,word:r.word,gloss:r.gloss,definition:r.definition,pos:r.pos,domainLabel:r.domain,rootWordsOnly:r.rootWordsOnly,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-search-words-result",{detail:{requestId:r.requestId,words:t.words,totalMatches:t.totalMatches}}))})})}searchPhrasesBridge(){document.addEventListener("lira-search-phrases",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.searchPhrases(this.currentVocabularyDomainName,{word:r.word,gloss:r.gloss,definition:r.definition,pos:r.pos,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-search-phrases-result",{detail:{requestId:r.requestId,phrases:t.phrases,totalMatches:t.totalMatches}}))})})}searchSensesBridge(){document.addEventListener("lira-search-senses",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.searchSenses(this.currentVocabularyDomainName,{word:r.word,gloss:r.gloss,definition:r.definition,pos:r.pos,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-search-senses-result",{detail:{requestId:r.requestId,senses:t.senses,totalMatches:t.totalMatches}}))})})}searchRelationshipsBridge(){document.addEventListener("lira-search-relationships",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.searchRelationships(this.currentVocabularyDomainName,{wordId:r.wordId,query:r.query,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-search-relationships-result",{detail:{requestId:r.requestId,relationships:t.relationships,totalMatches:t.totalMatches}}))})})}searchLexicalRelationshipsBridge(){document.addEventListener("lira-search-lexical-relationships",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.searchLexicalRelationships(this.currentVocabularyDomainName,{wordId:r.wordId,query:r.query,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-search-lexical-relationships-result",{detail:{requestId:r.requestId,relationships:t.relationships,totalMatches:t.totalMatches}}))})})}resolveHierarchyBridge(){document.addEventListener("lira-resolve-hierarchy",e=>{const r=e.detail;this.currentVocabularyDomainName&&this.vocabularyClient.resolveHierarchy(this.currentVocabularyDomainName,{kind:r.kind,wordId:r.wordId,limit:r.limit}).then(t=>{document.dispatchEvent(new CustomEvent("lira-resolve-hierarchy-result",{detail:{requestId:r.requestId,...t}}))})})}mount(e){this.container=e,document.title=this.title,this.ensureStyles(),e.addEventListener("click",r=>this.handleClick(r)),this.render()}handleClick(e){const r=e.target.closest("[data-action]");if(!r||r.disabled)return;const t=r.dataset.action;t==="select"?(this.selectedName=r.dataset.domain,this.mode==="mobile"&&(this.mobileScreen="view"),this.render()):t==="mode"?(this.mode=r.dataset.mode,this.mode==="mobile"&&(this.mobileScreen=this.selectedName?"view":"browse"),this.render()):t==="back"?(this.mobileScreen="browse",this.render()):t==="component"?(this.selectedComponent=r.dataset.component,this.render()):t==="toggle-tree"?(this.treeCollapsed=!this.treeCollapsed,this.render()):t==="seed-wordnet"?this.vocabularyClient.seedWordNet(E):t==="seed-common-vocabulary"&&this.vocabularyClient.seedCommonVocabulary(E)}renderVocabToolbar(){return`<div class="portal-vocab-toolbar">${this.vocabToolbarInner(this.statusBoard.get("vocabulary"))}</div>`}vocabToolbarInner(e){const r=(e==null?void 0:e.state)==="running",t=e==null?void 0:e.progress;return`
    ...
    `;const t=this.container.querySelector(".portal-service-status");t&&this.serviceStatusView.mount(t),e&&this.selectedComponent==="vocabulary"?this.loadView(e):e&&this.selectedComponent==="linguistics"&&this.loadLinguisticsView()}loadLinguisticsView(){if(!this.container)return;const e=this.container.querySelector(".portal-fragment-mount");e&&this.sentenceReaderView.mount(e)}async loadView(e){const r=++this.renderToken;this.setViewStatus("Loading Vocabulary…");let t;try{t=await this.vocabularyClient.renderDomain(e.name)}catch(i){if(r!==this.renderToken)return;this.setViewStatus(`Couldn't load this Domain's Vocabulary view: ${i instanceof Error?i.message:String(i)}`);return}if(r!==this.renderToken||!this.container)return;this.ensureFragmentStyles(t.style);const s=this.container.querySelector(".portal-fragment-mount");if(s){s.innerHTML=t.body,this.currentVocabularyDomainName=e.name;const i=document.createElement("script");i.textContent=`(function () {
${t.script}
})();`,s.appendChild(i)}this.setViewStatus(void 0)}setViewStatus(e){var t;const r=(t=this.container)==null?void 0:t.querySelector(".portal-view-status");r&&(r.textContent=e??"",r.style.display=e?"block":"none")}renderTopbar(e){const r=`
      <div class="portal-mode-toggle" role="group" aria-label="Layout">
        ...
        <button type="button" data-action="mode" data-mode="mobile" class="${this.mode==="mobile"?"active":""}">${oe} Mobile</button>
          ...
          <div class="portal-topbar">
            ...
            ${r}
          </div>`:`<div class="portal-topbar"><span class="portal-topbar-title">All Domains</span>${r}</div>`;const t=e?this.registry.ancestryOf(e.name):[];return`<div class="portal-topbar"><nav class="portal-breadcrumb">${[`<span class="crumb-root">${ne} All Domains</span>`,...t.map(i=>`<span>${h(i.name)}</span>`)].join(`<span class="crumb-sep">${se}</span>`)}</nav>${r}</div>`}renderBody(e){return this.mode==="desktop"?`${this.renderTree()}${this.renderViewPane(e)}`:this.mobileScreen==="browse"?`<div class="portal-tree portal-tree--mobile">${this.renderTreeRows(0)}</div>`:this.renderViewPane(e,!0)}renderTree(){return this.treeCollapsed?`
        <nav class="portal-tree portal-tree--collapsed">
          <button type="button" class="portal-tree-toggle" data-action="toggle-tree" title="Expand Domains" aria-label="Expand Domains" aria-expanded="false">${de}</button>
        </nav>`:`
      <nav class="portal-tree">
        <div class="portal-tree-label">
          <span>Domains</span>
          <button type="button" class="portal-tree-toggle" data-action="toggle-tree" title="Collapse Domains" aria-label="Collapse Domains" aria-expanded="true">${le}</button>
        </div>
        ${this.renderTreeRows(0)}
      </nav>`}renderTreeRows(e,r){return(e===0?this.registry.roots():this.registry.children(r??"")).map(s=>{const i=this.registry.children(s.name),a=s.name===this.selectedName;return`
          <div class="portal-tree-row depth-${e} ${a?"selected":""}" data-action="select" data-domain="${h(s.name)}">
            ${i.length>0?te:'<span class="chev-spacer"></span>'}
            ${re}
      ...
      <div class="portal-component-switcher" role="tablist" aria-label="UI Component">
        ...
        `).join("")}
      </div>`}renderViewPane(e,r=!1){const t=this.renderComponentSwitcher(),s='<div class="portal-service-status"></div>',i=this.selectedComponent==="vocabulary"?this.renderVocabToolbar():"";if(!e)return`
        <div class="portal-view ${r?"portal-view--full":""}">
          ${t}
          ${i}
          <div class="portal-view-empty">Select a Domain to continue.</div>
          ${s}
        </div>`;const a=C.find(p=>p.id===this.selectedComponent),d=a!=null&&a.available?`
      ...
      <div class="portal-view ${r?"portal-view--full":""}">
        ...
        ${d}
        ${s}
      </div>`}}function h(n){return n.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}const re='<svg class="i-folder" viewBox="0 0 16 16" fill="currentColor"><path d="M1.5 3.5A1 1 0 0 1 2.5 2.5h3.6l1.2 1.4H13.5A1 1 0 0 1 14.5 5v7A1 1 0 0 1 13.5 13h-11a1 1 0 0 1-1-1v-8.5z"/></svg>',te='<svg class="i-chev" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6l4 4 4-4"/></svg>',se='<svg class="i-chev-right" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 3l5 5-5 5"/></svg>',ne='<svg class="i-home" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><path d="M2 8l6-5 6 5M4 7v6h8V7"/></svg>',ie='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 3L5 8l5 5"/></svg>',ae='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="1.5" y="2.5" width="13" height="9" rx="1"/><path d="M6 13.5h4"/></svg>',oe='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.4"><rect x="4.5" y="1.5" width="7" height="13" rx="1.4"/></svg>',le='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M10 3L5 8l5 5"/></svg>',de='<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="2"><path d="M6 3l5 5-5 5"/></svg>',ce=`
.portal-shell {
