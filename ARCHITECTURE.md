...
| Architect Rule ID | Architecture Rule Statement Text | Description Text | Example Text | Pattern Text |
|---|---|---|---|---|
| DNC-001 | A Domain Name must use reverse-hierarchy notation. | The most specific domain component shall be placed on the left and the universal root shall be placed on the right. The hierarchy is therefore read from right to left. | `python.programming.language.common` | `<specific>.<parent>.<root>` |
| DNC-002 | A Domain Name must end with the applicable root domain. | Domains belonging to LIRA's shared hierarchy terminate with `common`. | `language.common`, `physics.science.common` | `<domain-path>.common` |
| DNC-003 | A Domain Name must ensure that each component semantically specialises the component immediately to its right. | Each component moving left narrows the meaning of its parent. Unrelated organisational or technical groupings shall not be inserted. | `python.programming.language.common` | `<specialisation>.<generalisation>` |
| DNC-004 | A Domain Name must use `language.common` for LIRA's default natural language hierarchy. | English is LIRA's default common language and is therefore implicit. | `noun.language.common`, `punctuation.language.common` | `<language-category>.language.common` |
| DNC-005 | A Domain Name must not include the ISO language code `en` for LIRA's default natural language. | The English language code is omitted because English is the default language of the common vocabulary. | `noun.language.common` | `<category>.language.common` |
| DNC-006 | A Domain Name must include the ISO language code immediately to the left of `language` for a non-default natural language. | The ISO language code identifies vocabulary, grammar or character usage specific to a natural language. | `noun.fr.language.common`, `letter.ar.language.common` | `<category>.<ISO-language-code>.language.common` |
| DNC-007 | A Domain Name must use a consistent ISO 639 language code convention. | A single ISO language-code convention shall be adopted across LIRA, preferably ISO 639-1 where available. | `fr`, `de`, `ar`, `ja` | `<ISO-language-code>.language.common` |
| DNC-008 | A Domain Name must model programming languages beneath `programming.language.common`. | Programming languages are modelled as a specialisation of language. | `python.programming.language.common`, `csharp.programming.language.common` | `<programming-language>.programming.language.common` |
| DNC-009 | A Domain Name must place shared Unicode character categories beneath `language.common`. | Unicode character categories whose script context is Common belong in the shared language hierarchy. | `punctuation.language.common`, `symbol.language.common`, `digit.language.common` | `<Unicode-category>.language.common` |
| DNC-010 | A Domain Name must include the ISO language code for Unicode character categories specific to a non-default natural language. | Language-specific character sets or usages are separated using the ISO language code. | `letter.ar.language.common`, `punctuation.ja.language.common` | `<Unicode-category>.<ISO-language-code>.language.common` |
| DNC-011 | A Domain Name must not duplicate shared lexical items in specialised domains unless their meaning or usage is specialised. | Words or characters shall exist in the highest applicable domain. Specialised domains contain only additions, overrides or specialised behaviour. | `<!>` exists only in `punctuation.language.common`. | `<shared-category>.language.common` |
| DNC-012 | A Domain Name must participate in the unique identity of a word. | Within LIRA a word is uniquely identified by the combination of Domain and Unicode Character Sequence. | `<+>` in `mathematics.language.common` differs from `<+>` in `python.programming.language.common`. | `Word Identity = Domain + Unicode Character Sequence` |
| DNC-013 | A Domain Name must classify semantic context rather than physical storage or deployment. | Domain names describe meaning. Storage, deployment, services and ownership are separate architectural concerns. | `python.programming.language.common` instead of `python.assets.vocabulary.service.common`. | `<semantic-domain-path>.common` |
| DNC-014 | A Domain Name must use singular canonical terms unless the component is a governed standard code. | Domain components should be singular, lowercase and unambiguous. Standard identifiers such as ISO language codes are permitted. | `language`, `programming`, `punctuation` | `<singular-lowercase-component>` |
| DNC-015 | A Domain Name must not encode character, grammatical, operational or system properties as hierarchy components. | Unicode category, Unicode script, case, number, version, confidence, provenance and activation are metadata unless they represent a semantic domain. | `letter.ar.language.common` with `UnicodeScript=Arabic` instead of `letter.arabic.ar.language.common`. | `<semantic-domain-path> + metadata` |
| DNC-016 | A Domain Name must not include version information by default. | Versions should normally be modelled as metadata. A version-specific domain is created only when versions represent semantically distinct vocabularies or grammars. | `python.programming.language.common` with `Version=3.14`. | `<language>.programming.language.common` |
| DNC-017 | A Domain Name must be semantically resolvable from right to left. | Each component shall answer the question: "What kind of the component to its right is this?" | `common` → `language` → `programming` → `python` | `<leaf>.<specialisation>...<root>` |
| DNC-018 | A Domain Name must not reverse the parent-specialisation order. | The leaf is written first and the root last. A parent shall never appear to the left of its child. | `python.programming.language.common` instead of `programming.python.language.common` | `<child>.<parent>.<root>` |
