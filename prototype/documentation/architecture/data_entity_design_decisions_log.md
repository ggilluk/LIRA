...
### `WordSeeder.seedClosedClassWords()` calls `DeterminerSeeder` internally, unlike `AuxiliarySeeder`

`AuxiliarySeeder` is a deliberately separate, caller-invoked step (`vocabulary_worker.ts`'s own `handleSeedCommonVocabulary`) — a caller of `seedClosedClassWords()` that doesn't also call `AuxiliarySeeder` simply gets no AUXILIARY coverage, an accepted gap (`linguistics_worker.ts`'s own `handleInit()` is exactly this case today). `DeterminerSeeder` was first wired the same way, then folded directly into `seedClosedClassWords()` itself once real usage disagreed: every one of that method's own callers — this codebase's own tests very much included, not just `vocabulary_worker.ts` — already expects a real DETERMINER Word for "the"/"a"/... the moment it returns, the same as every other closed class `loadCache()` still seeds directly. Confirmed empirically, not just reasoned about: pulling `determiners.json` out of `MANDATORY_FILES` without also folding `DeterminerSeeder` into `seedClosedClassWords()` broke 13 existing tests outright.
