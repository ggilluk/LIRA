...
Read back from `WordSeeder`'s own seeding-time-only `LexicalRelationship` graph (`VocabularyContext`'s own docstring, `data/vocabulary_context.ts`, on why nothing outside a seeder reads that graph directly any more) rather than left as a queryable `CONTRACTION` edge. Word-level, not a POS-subtype field, since a contraction's own components span whatever closed-class parts of speech happen to combine (pronoun + auxiliary verb, negation particle, modal, ...), never one single POS the way a derivation pair's own fields are scoped. Many-to-many, not a single pointer — every Orthographic-group kind, `CONTRACTION` included, carries zero real seeded data as of `asset_version 1.22.0` (the Common Vocabulary Relationship Cache's own `orthographic_relationships.json` emptied when `PartOfSpeech.PARTICLE` was retired and its sole entry, `not`/`n't`, became unresolvable — `assets/common/en/relationships/README.md` — leaving `SPELLING_VARIANT`, `ABBREVIATION`, `ACRONYM`, ... with no attribute field built for any Orthographic kind yet). Deliberately one-directional — a component word's own reverse index of every contraction it participates in isn't built here; nothing reads it, and it can be added later without touching this field.
...
Was a loose string at first, deliberately not a new enum — matching how `WordFormRow.field` itself was also a loose string at the time. Once every POS subtype had migrated onto real `WordForm` records (this section's own second paragraph above) and the Matrix's own row set stopped growing, that loose-string stage was retired in favour of `WordFormField` (`data/enums/word_forms_enum.ts`) — the single closed enum both `WordForm.field` and `WordFormRow.field` now share, generated directly from every field name the Matrix actually declares (27 members, one per row, `WordFormField`'s own docstring has the full list). String-valued rather than tensor-coded like every other enum in this folder (PartOfSpeech, RegisterCode, ...) — `field` is a lookup key and a value real client-facing code already reads directly (`ui/server/builder_word.ts`'s own `WordFormEntry.field`, a DTO that stays a plain `string` on the other side of that serialisation boundary), not a system-tensor dimension, so there's no numeric code to assign it; each member's own value is the exact camelCase string every existing consumer already read and wrote, unchanged — the enum adds compile-time enforcement of an already-agreed set, not a new runtime representation. `stringPatternsFor(field, pos)` validates a WordForm's own `text` against the identical matrix rules a scalar field's value used to be checked against; the matrix doesn't care which shape holds the value.
...
Each of these four classes carries no field of its own beyond `partOfSpeech` — the Word Form to Part of Speech Matrix (`data/matrices/word_form_part_of_speech_matrix.md`) ticks only Base Lemma Canonical Form for each of these parts of speech, already `Word.baseLemmaCanonicalForm`'s own field, shared by every subtype. Each class exists purely so a caller can narrow a `Word` to "definitely a preposition/conjunction/interjection/numeral" at the type level, the same as its siblings.

Particle used to be a fifth class in this same group, with the same shape (no field beyond `partOfSpeech`). Removed entirely in `asset_version 1.27.0` along with `PartOfSpeech.PARTICLE` itself — `assets/common/en/README.md`'s `particles.json` entry has the full removal rationale.
