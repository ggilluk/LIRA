...
`morphological_relationships.json` above, except `not → n't`, removed
from `orthographic_relationships.json` in `asset_version 1.22.0`
alongside `PartOfSpeech.PARTICLE`'s retirement, and `due to → owing to`,
removed from `semantic_relationships.json` in `asset_version 1.23.0`
once `prepositions.json` dropped both `due to` and `owing to` entirely
(`../README.md`'s own Preposition senses section).
