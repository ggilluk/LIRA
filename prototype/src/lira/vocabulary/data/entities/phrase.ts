...
export interface Phrase extends LinguisticUnit {
   ...
   * sense sharing the same lexical form and part of speech -- a
   * graph-reference pointer to the one `Domain` (data/entities/domain.ts)
   * this fact names, resolved against a `Domains` store
   * (`Domains.findByUuid()`), not an embedded `Text` copy of the tag
   * itself (Word.domainTag's own identical shape and reasoning,
