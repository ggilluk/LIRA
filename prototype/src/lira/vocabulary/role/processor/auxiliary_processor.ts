 ...
 * violation the way recogniseVerbFormIssues can. Unlike every other POS subtype's
...
export function recogniseAuxiliaryFormIssues(auxiliary: Auxiliary, wordForms: WordForms): readonly WordFormIssue[] {
  const issues: WordFormIssue[] = [];
  for (const form of wordForms.formsOf(auxiliary)) {
    const issue = recogniseFormTextIssue(form.formType, form.text, stringPatternsFor(form.formType, PartOfSpeech.AUXILIARY));
