...
// recogniseLemmaPhraseType()'s own closed class of single-word prepositions --
...
// governing preposition) -- exactly why recogniseLemmaPhraseType only applies
...
// isDeterminerPhrase()'s own small denylist of "a "-led lemmas
...
// denylisted here still gets classified, just via recogniseLemmaPhraseType's
 ...
 * mismatch the PREPOSITIONAL_PHRASE check in recogniseLemmaPhraseType() below
 * already corrects for. `nounLemmas` (recogniseLemmaPhraseType()'s own
 ...
 * PHRASE_TYPE_DETERMINERS set recogniseModifierRoles() below uses --
...
function isDeterminerPhrase(tokens: readonly string[], lemma: string, nounLemmas: ReadonlySet<string>): boolean {
 ...
 *   degree adverbs -- isDeterminerPhrase() below.
 *
...
// recogniseModifierRoles()'s own closed set of core English determiners --
...
// recogniseModifierRoles()'s own closed set of English adverbs that
 ...
 * purposes of recogniseModifierRoles() below -- deliberately a *set*, not
 * the single arbitrary homograph `dictionary.lookup` alone would pick
 * (updatePhraseWordLinks()'s own docstring on why that single pick is
 ...
 * updatePhraseWordLinks()'s own "of" example) -- via the same two closed sets
 * recogniseLemmaPhraseType() itself already relies on for exactly this reason
 * (PHRASE_TYPE_PREPOSITIONS, PHRASE_TYPE_DETERMINERS above). */
function recogniseTokenPartOfSpeechCandidates(token: string, dictionary: Dictionary): ReadonlySet<PartOfSpeech> {
 ...
 * one). Shared by recogniseModifierRoles() below -- deciding *which
 * token position* is Head -- and by updatePhraseWordLinks()'s own
 * recogniseMatchingTokenHomograph() -- deciding *which of that position's own
 ...
 * to the wrong homograph: recogniseModifierRoles() correctly found "few"
 * Noun-capable via recogniseTokenPartOfSpeechCandidates()'s own full homograph set once
 * WordNet's own standalone NOUN sense for "few" existed, but
 * updatePhraseWordLinks() went on to resolve the position via
 ...
 * recogniseModifierRoles() itself actually matched. */
function identifyHeadTargetPartsOfSpeech(phraseType: PhraseType): ReadonlySet<PartOfSpeech> {
  switch (phraseType) {
...
function recogniseGeneralHeadIndex(possiblePos: readonly ReadonlySet<PartOfSpeech>[], targetPos: ReadonlySet<PartOfSpeech>): number | undefined {
 ...
 * instead of `-1`. */
function recogniseFirstPartOfSpeechIndex(possiblePos: readonly ReadonlySet<PartOfSpeech>[], targetPos: ReadonlySet<PartOfSpeech>, from: number): number | undefined {
 ...
 *   closed-set membership `recogniseLemmaPhraseType()` itself already checks
 ...
 *   Head Modifier rule `identifyNonHeadModifierRole()` below still carries for
 ...
 *   branches (`recogniseGeneralHeadIndex`'s own docstring). */
function recogniseComplementStartIndex(phraseType: PhraseType | undefined, tokens: readonly string[], headIndex: number | undefined): number | undefined {
 ...
 * once `recogniseComplementStartIndex()` above has already decided a span
 ...
 * constituency parsing, unlike `recogniseLemmaPhraseType()`'s own top-level
...
function recogniseComplementPhraseType(tokens: readonly string[]): PhraseType {
 ...
 * DETERMINER-role `tokens` should be built as, once `updatePhraseWordLinks()`
 * has already decided the run doesn't collapse. Structural, the same
 * "never read off a WordNet-tagged part of speech" reasoning
 * `recogniseComplementPhraseType()` above already has -- there is none
 ...
 * `recogniseAdverbHeadIndex()` below and `recogniseGeneralHeadIndex()`'s
 * own fallback branch already have, not a claim this always matches true
 * syntactic structure. */
function recogniseModifierPhraseType(tokens: readonly string[], dictionary: Dictionary): PhraseType {
  const possiblePos = tokens.map((token) => recogniseTokenPartOfSpeechCandidates(token, dictionary));
...
function recogniseCoordinatingConjunctionIndex(tokens: readonly string[], dictionary: Dictionary): number | undefined {
 ...
 * `recogniseGeneralHeadIndex` decides it, except for the one
 ...
 * recogniseLemmaPhraseType's own PHRASE_TYPE_PREPOSITIONS/
 * INFINITIVE_LOOKALIKE_DENYLIST do; falls through to the shared rule
 * for every other Adverb Phrase shape. */
function recogniseAdverbHeadIndex(possiblePos: readonly ReadonlySet<PartOfSpeech>[], tokens: readonly string[]): number | undefined {
  ...
  return recogniseGeneralHeadIndex(possiblePos, identifyHeadTargetPartsOfSpeech(PhraseType.ADVERB_PHRASE));
}

/** Assigns a ModifierRole (enums/modifier_role.ts) to every one of `tokens`
 * -- called from updatePhraseWordLinks() below, over the same token list its
 * own local `words` resolves, so the two stay index-aligned; neither is
 * stored back onto the Phrase itself (updatePhraseWordLinks()'s own docstring
 * on why). Implements
 ...
 * parts of speech (recogniseTokenPartOfSpeechCandidates() above -- not one arbitrarily-
 ...
 * same "Definition" Common Rule constraint `recogniseLemmaPhraseType` above
 ...
 * alone (recogniseAdverbHeadIndex's own docstring) -- resolved for the one
 ...
 *   recogniseGeneralHeadIndex/recogniseAdverbHeadIndex. Every
 ...
 *   genuine Complement span (`recogniseComplementStartIndex()` above) instead
 ...
 *   instead (`updatePhraseWordLinks()`'s own docstring on why).
 ...
 *   embedded Complement instead (`recogniseComplementStartIndex()` above always
 ...
 * own `determiners`, not this one's (`updatePhraseWordLinks()`'s own
...
export function recogniseModifierRoles(phraseType: PhraseType | undefined, tokens: readonly string[], dictionary: Dictionary): readonly (ModifierRole | undefined)[] {
  ...
  switch (phraseType) {
    case PhraseType.NOUN_PHRASE:
    case PhraseType.ADJECTIVE_PHRASE:
      headIndex = recogniseGeneralHeadIndex(possiblePos, identifyHeadTargetPartsOfSpeech(phraseType));
      break;
    case PhraseType.ADVERB_PHRASE:
      headIndex = recogniseAdverbHeadIndex(possiblePos, tokens);
      break;
    case PhraseType.VERB_PHRASE:
      headIndex = recogniseFirstPartOfSpeechIndex(possiblePos, identifyHeadTargetPartsOfSpeech(phraseType), 0);
      break;
    case PhraseType.PREPOSITIONAL_PHRASE:
      headIndex = recogniseFirstPartOfSpeechIndex(possiblePos, identifyHeadTargetPartsOfSpeech(phraseType), 0);
      break;
  ...
  const complementStart = recogniseComplementStartIndex(phraseType, tokens, headIndex);
  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    roles[i] = identifyNonHeadModifierRole(phraseType, possiblePos, i, headIndex);
  }
  return roles;
}
 ...
 * recogniseModifierRoles()'s own per-PhraseType Word Role Assignment
 ...
 * a role to. */
function identifyNonHeadModifierRole(
 ...
 * DETERMINER run is allowed to resolve against -- `identifyNonHeadModifierRole()`'s
 ...
 * `createCoordinateSide()` (below) needs to resolve each coordinate's own
 ...
 * set the way `recogniseModifierPhraseType()` does for choosing a brand
 * new nested Phrase's own `phraseType` (a different concern -- that
 * nested Phrase gets its own full recursive `updatePhraseWordLinks()` pass
 ...
 * DETERMINER (`createModifierRunValue()`'s only two callers, `updatePhraseWordLinks()`);
 ...
 * case `identifyNonHeadModifierRole()` itself splits by position (ADVERB before
 ...
 * itself `undefined`) --
 * `recogniseMatchingTokenHomograph()`'s own graceful "falls back to the first homograph"
 * behavior for an empty target set, `createCoordinateSide()`'s own
 * unchanged reasoning either way. */
function identifyModifierRunTargetPos(phraseType: PhraseType | undefined, role: ModifierRole.MODIFIER | ModifierRole.DETERMINER, isPreHead?: boolean): ReadonlySet<PartOfSpeech> {
 ...
 * recogniseModifierRoles()'s own per-token role) are local to this
 ...
 * arbitrary-but-deterministic choice recogniseDefinitionWords() already makes
 ...
 * own "of" -- as a standalone sense). recogniseModifierRoles()
 * deliberately doesn't reuse this single arbitrary-homograph
 * resolution, though -- it resolves every token's own full *set* of
 * possible parts of speech instead (recogniseTokenPartOfSpeechCandidates(), this
 ...
 * `words[headIndex]` is resolved via `recogniseMatchingTokenHomograph()` below, not the
 ...
 * one actually matching `identifyHeadTargetPartsOfSpeech(phrase.phraseType)`,
 * the identical target recogniseModifierRoles() itself already matched
 ...
 * `identifyHeadTargetPartsOfSpeech` actually meant.
 *
 ...
 * `recogniseTokenWordFormId()` further below is its own standalone
 ...
 * run of same-role tokens (`createModifierRunValue()` below) -- MODIFIER
 * tokens split into a pre-Head run and a post-Head run
 * (`recognisePreHeadModifierRun()`/`recognisePostHeadModifierRun()`), DETERMINER tokens
 * into one run regardless of position (`recogniseDeterminerRun()`, the Common
 ...
 * `Identifier` (`recogniseTokenWordFormId()`). A run of two or more tokens
 * is where this now genuinely differs from before: it becomes one
 * nested Phrase (`createLinkedNestedPhrase()`, `recogniseModifierPhraseType()`'s
 * own docstring on which PhraseType) or, when a genuine coordinating
 * conjunction sits inside the run ("big and red"), one real
 * `Coordination` (`recogniseCoordinatingConjunctionIndex()`/
 * `createStoredModifierCoordination()`) -- the reported fix: "attributive
 ...
 * first token alone, `recogniseTokenWordFormId()`'s plain single-token
 ...
 * identical `createLinkedNestedPhrase()` mechanism `preModifier`/`postModifier`/
 * `determiner` now share: when `recogniseModifierRoles()` finds a
 * ModifierRole.COMPLEMENT position (`recogniseComplementStartIndex()`'s own
 * docstring on exactly which token, per PhraseType), every token from
 * there to the end of `tokens` becomes one nested Phrase (a
 * PrepositionalPhrase or NounPhrase, `recogniseComplementPhraseType()`'s
 ...
 * (`createStoredNestedPhrase()`), so it becomes its own real, independently-
 ...
 * found-or-created directly in it (`createStoredModifierCoordination()`).
 ...
 * `updatePhraseWordLinks()`'s own Head-position fix, `recogniseTokenPartOfSpeechCandidates()`'s
 ...
 * two synthetic closed-set memberships `recogniseTokenPartOfSpeechCandidates()` itself
...
function recogniseMatchingTokenHomograph(token: string, targetPos: ReadonlySet<PartOfSpeech>, dictionary: Dictionary): Word | undefined {
 ...
 * `updatePhraseWordLinks()`'s own local `matchingFormId()` closure for a
 ...
 * function that isn't itself a closure over `updatePhraseWordLinks()`'s own
 * `words`/`tokens`, since `createModifierRunValue()` below is called with an
...
function recogniseTokenWordFormId(token: string, dictionary: Dictionary, wordForms?: WordForms): Identifier | undefined {
 ...
 * carry over (`recogniseComplementPhraseType()`/`recogniseModifierPhraseType()`'s
 ...
 * `recogniseLemmaPhraseType()`'s own NOUN -> NOUN_PHRASE default one Phrase-
...
function identifySyntheticPartOfSpeech(phraseType: PhraseType): PartOfSpeech {
  switch (phraseType) {
    case PhraseType.PREPOSITIONAL_PHRASE:
 ...
 * dispatch `createStoredNestedPhrase()`/`createLinkedNestedPhrase()` below both
 ...
 * `identifySyntheticPartOfSpeech()` above assigns) rather than appending a
 ...
 * idempotent: `updatePhraseWordLinks()` genuinely runs more than once over the
 ...
 * gets recursively re-linked below (`createLinkedNestedPhrase()`'s own caller)
 ...
 * pass. */
function createStoredNestedPhrase(phrases: Phrases, text: string, phraseType: PhraseType): Phrase {
  const partOfSpeech = identifySyntheticPartOfSpeech(phraseType);
 ...
 * `updatePhraseWordLinks()`'s own docstring on both call shapes. `phraseType`
 * is decided by the caller (`recogniseComplementPhraseType()` for a
 * Complement span, `recogniseModifierPhraseType()` for a Modifier/
 ...
 * `createStoredNestedPhrase()` above, so it becomes its own real,
 * independently-listed entry in the Phrases store; when omitted
 * (matching every other optional-store convention `updatePhraseWordLinks()`
 ...
 * its parent's own fields. Either way, the recursive `updatePhraseWordLinks()`
...
function createLinkedNestedPhrase(
  ...
  const nested = phrases !== undefined ? createStoredNestedPhrase(phrases, text, phraseType) : createNestedPhrase(phraseType, text);
  updatePhraseWordLinks(nested, dictionary, wordForms, phrases, coordinations);
  return nested;
}

/** One side of a coordinated Modifier/Determiner run ("big" or "red" in
 * "big and red") -- length 1 resolves via `recogniseMatchingTokenHomograph()` against
 * `targetPos` (the same POS-aware resolution the Head position itself
 * gets, more precise than `recogniseTokenWordFormId()`'s plain first-
 ...
 * via `createLinkedNestedPhrase()`, one level only -- a coordinate side is
 * never itself searched for a second, nested coordination (no real
 * bundled data ever exercises this, `recogniseCoordinatingConjunctionIndex()`'s
 * own docstring). `undefined` only when `tokens` is empty, which
 * `recogniseCoordinatingConjunctionIndex()`'s own bounds (`1 <= i <= length - 2`)
 * already guarantee never happens for either side. */
function createCoordinateSide(
  tokens: readonly string[],
  ...
  if (tokens.length === 1) return recogniseMatchingTokenHomograph(tokens[0], targetPos, dictionary);
  return createLinkedNestedPhrase(tokens, recogniseModifierPhraseType(tokens, dictionary), dictionary, wordForms, phrases, coordinations);
}

/** `entry`'s own graph-identity uuid, whichever of Word's own `wordId`
 * or Phrase's own `phraseId` it actually carries -- `createStoredModifierCoordination()`'s
 ...
 * `createStoredNestedPhrase()`'s own reuse-before-create shape, adapted to
 ...
 * matched Conjunction Word (`recogniseTokenWordFormId()`) -- this function
 * never registers a new WordForm, `updatePhraseWordLinks()`'s own identical
 * invariant. */
function createStoredModifierCoordination(
  coordinations: Coordinations<LinguisticUnit>,
  ...
  // (this function's own real callers, `createModifierRunValue()`), so the
 ...
 * `updatePhraseWordLinks()`'s own docstring on when each of the three calls
 * this, and on the one determiner-only exception it never reaches this
 * function for. Length 1 -> `recogniseTokenWordFormId()` (unchanged from
 * before this feature). Length 2 or more -> `recogniseCoordinatingConjunctionIndex()`
 * first: found -> split the run around it, `createCoordinateSide()`
 * each half against `targetPos` (`identifyModifierRunTargetPos()`'s own docstring
 * on why this, not `recogniseModifierPhraseType()`, decides each
 ...
 * tokens' own ambiguous homograph set), `createStoredModifierCoordination()`
 * (or a bare, unregistered `createCoordination()` when `coordinations`
 * is omitted); not found -> `recogniseModifierPhraseType()` +
 * `createLinkedNestedPhrase()`, one flat nested Phrase for the whole run --
 ...
 * own full recursive `updatePhraseWordLinks()` pass afterward to self-correct
 * any imprecision here. `undefined` when `tokens` is empty (no run at
 * all). */
function createModifierRunValue(
  tokens: readonly string[],
  targetPos: ReadonlySet<PartOfSpeech>,
  dictionary: Dictionary,
  wordForms?: WordForms,
  ...
  if (tokens.length === 0) return undefined;
  if (tokens.length === 1) return recogniseTokenWordFormId(tokens[0], dictionary, wordForms);

  const conjunctionIndex = recogniseCoordinatingConjunctionIndex(tokens, dictionary);
  if (conjunctionIndex !== undefined) {
    ...
    const left = createCoordinateSide(leftTokens, targetPos, dictionary, wordForms, phrases, coordinations);
    const right = createCoordinateSide(rightTokens, targetPos, dictionary, wordForms, phrases, coordinations);
    if (left !== undefined && right !== undefined) {
      ...
      const coordinatorId = coordinatorWord !== undefined ? recogniseTokenWordFormId(tokens[conjunctionIndex], dictionary, wordForms) : undefined;
      return coordinations !== undefined
        ? createStoredModifierCoordination(coordinations, [left, right], coordinatorId)
 ...
 * Word -- `recogniseCoordinatingConjunctionIndex()`'s own membership check, pulled
 * out as its own predicate so `recogniseBackwardRoleRunBoundary()`/`recogniseForwardRoleRunBoundary()`
 ...
 * a MODIFIER run) -- `recogniseModifierRoles()` never assigns MODIFIER or
 * DETERMINER to a Conjunction-only token itself (`identifyNonHeadModifierRole()`'s
 * own switch has no Conjunction-role branch, `recogniseTokenPartOfSpeechCandidates()`
 * never returns CONJUNCTION alongside NOUN/ADJECTIVE/ADVERB/DETERMINER
 * for any of "and"/"or"/"nor"/"but"), so without this bridge a
 * coordinator embedded in a run would stop `recognisePreHeadModifierRun()`/
 * `recognisePostHeadModifierRun()`/`recogniseDeterminerRun()` right at its own position,
 * the same way any other non-`role` token does -- `createModifierRunValue()`'s
 * own `recogniseCoordinatingConjunctionIndex()` split would then never see it,
 ...
 * either side, matching `recogniseCoordinatingConjunctionIndex()`'s own binary-
 * split-only scope ("X and Y", never a comma-joined "X, Y, and Z"). */
function recogniseBackwardRoleRunBoundary(wordRoles: readonly (ModifierRole | undefined)[], tokens: readonly string[], dictionary: Dictionary, start: number, role: ModifierRole): number {
  while (start > 0) {
...
/** `recogniseBackwardRoleRunBoundary()`'s own forward counterpart. */
function recogniseForwardRoleRunBoundary(wordRoles: readonly (ModifierRole | undefined)[], tokens: readonly string[], dictionary: Dictionary, end: number, role: ModifierRole): number {
 ...
 * run is genuinely contiguous -- but `recogniseBackwardRoleRunBoundary()`'s own bridging
...
function recognisePreHeadModifierRun(wordRoles: readonly (ModifierRole | undefined)[], headIndex: number, tokens: readonly string[], dictionary: Dictionary): [number, number] | undefined {
  if (headIndex <= 0 || wordRoles[headIndex - 1] !== ModifierRole.MODIFIER) return undefined;
  return [recogniseBackwardRoleRunBoundary(wordRoles, tokens, dictionary, headIndex - 1, ModifierRole.MODIFIER), headIndex];
}

/** `recognisePreHeadModifierRun()`'s own post-Head counterpart. */
function recognisePostHeadModifierRun(wordRoles: readonly (ModifierRole | undefined)[], headIndex: number, tokens: readonly string[], dictionary: Dictionary): [number, number] | undefined {
  if (headIndex === -1 || headIndex >= wordRoles.length - 1 || wordRoles[headIndex + 1] !== ModifierRole.MODIFIER) return undefined;
  return [headIndex + 1, recogniseForwardRoleRunBoundary(wordRoles, tokens, dictionary, headIndex + 1, ModifierRole.MODIFIER) + 1];
}
 ...
 * carries the role at all. Unlike `recognisePreHeadModifierRun()`/
 * `recognisePostHeadModifierRun()`, this never gates on a Head position (the
 ...
 * bundled multi-token case), extended the same `recogniseForwardRoleRunBoundary()` way
 * as `recognisePostHeadModifierRun()` across a real coordinating conjunction
 * ("his or her", never a real bundled example either, but structurally
 * identical to a coordinated Modifier run). */
function recogniseDeterminerRun(wordRoles: readonly (ModifierRole | undefined)[], tokens: readonly string[], dictionary: Dictionary): [number, number] | undefined {
  const start = wordRoles.indexOf(ModifierRole.DETERMINER);
  if (start === -1) return undefined;
  return [start, recogniseForwardRoleRunBoundary(wordRoles, tokens, dictionary, start, ModifierRole.DETERMINER) + 1];
}

export function updatePhraseWordLinks(
  phrase: Phrase,
  dictionary: Dictionary,
  wordForms?: WordForms,
  phrases?: Phrases,
  ...
  const wordRoles = recogniseModifierRoles(phrase.phraseType, tokens, dictionary);
  ...
  phrase.complements =
    complementIndex === -1
      ? []
      : [createLinkedNestedPhrase(tokens.slice(complementIndex), recogniseComplementPhraseType(tokens.slice(complementIndex)), dictionary, wordForms, phrases, coordinations)];

  const preRun = recognisePreHeadModifierRun(wordRoles, headIndex, tokens, dictionary);
  phrase.preModifier =
    preRun === undefined
      ? undefined
      : createModifierRunValue(tokens.slice(preRun[0], preRun[1]), identifyModifierRunTargetPos(phrase.phraseType, ModifierRole.MODIFIER, true), dictionary, wordForms, phrases, coordinations);

  const postRun = recognisePostHeadModifierRun(wordRoles, headIndex, tokens, dictionary);
  phrase.postModifier =
    postRun === undefined
      ? undefined
      : createModifierRunValue(tokens.slice(postRun[0], postRun[1]), identifyModifierRunTargetPos(phrase.phraseType, ModifierRole.MODIFIER, false), dictionary, wordForms, phrases, coordinations);

  const detRun = recogniseDeterminerRun(wordRoles, tokens, dictionary);
  if (detRun === undefined) {
    ...
    // -- `updatePhraseWordLinks()`'s own docstring on why ("each other" would
    // otherwise recurse forever into an identical child).
    phrase.determiner = recogniseTokenWordFormId(tokens[detRun[0]], dictionary, wordForms);
  } else {
    phrase.determiner = createModifierRunValue(tokens.slice(detRun[0], detRun[1]), identifyModifierRunTargetPos(phrase.phraseType, ModifierRole.DETERMINER), dictionary, wordForms, phrases, coordinations);
  }
}
