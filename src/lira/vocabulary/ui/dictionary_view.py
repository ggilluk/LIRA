...
<div class="page">
  ...
  <section class="panel active" id="panel-words">
    <div class="words-layout">
      <aside class="detail-panel">
        <div class="detail-empty" id="detail-empty">Select a word below to see its relationships.</div>
  ...
  <section class="panel" id="panel-hierarchy">
    <aside class="detail-panel" style="max-height:none">
...
const state = { tab: "words", query: "", pos: "", domain: "", selectedWordId: null, hierarchyKind: null, sort: { words: ["lexical_form", 1], rels: ["source_text", 1] } };
...
function renderHierarchy() {
  ...
  container.querySelectorAll("button[data-pivot-id]").forEach(btn => {
    btn.addEventListener("click", () => {
      selectTab("words");
      selectWord(btn.dataset.pivotId);
      const row = document.querySelector(`#words-body tr[data-word-id="${btn.dataset.pivotId}"]`);
