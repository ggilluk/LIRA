...
Split 13 Common Vocabulary Cache entries whose one `definition` had
secretly merged two or more distinct senses -- found by noticing the
Cyclic tab's SYNONYM box for a word pull in relationship lines that
belonged to an entirely different meaning of it. Select `character`
(Common, Noun) in the Words tab: there are now two entries, one tagged
`quality.common` ("The mental and moral qualities distinctive of an
individual", grouped with `disposition`/`nature`/`spirit`) and one
tagged `mark.common` ("A symbol used in writing or printing", grouped
with `symbol`/`letter`) -- previously one entry with both senses
jammed into a single definition, and both sets of relationships
attached to it as if they described the same thing. The Domain filter
dropdown now lists every subdomain a split produced (`symbol.common`,
`item.common`, `category.common`, ...) alongside plain `Common` and
`Physics`. See `vocabulary/assets/common/en/README.md`'s Polysemous
senses section for the full account of all 13 words and the new
`Word.domain_tag` mechanism behind it.

Regenerated by the same command as the section above --
`common_semantic_completion_seeding.py`'s own `run()` calls
`examples/common_polysemy_split_seeding.py`'s `run()` as its last
step, so this file already reflects the split.
