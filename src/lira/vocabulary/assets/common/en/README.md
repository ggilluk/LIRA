...
| Field | Populated? |
|-------|-------------|
| `entry_id` | Always -- a stable, generated-once identifier (Word 4.2, `entry_id`) unique across the entire cache, checked by `WordSeeder.validate_assets()`. Distinct from the in-memory `Word.uuid`, which is regenerated every time this entry is copied into a Domain's Dictionary |
...
4.2's `entry_id` field, the "Which fields are populated" table above)
-- a real UUID, generated once and stored, that names one distinct
(domain, part_of_speech, lexical_form) vocabulary entry for as long as
it exists, independent of `Word.uuid` (which stays a per-Domain-graph-
instance identity, regenerated on every copy). Motivated by a UI
usability issue: `DictionaryProcessor.register_conflicting_sense`
(9.2's Strategy 3) used to disambiguate two coexisting senses of the
same (text, part_of_speech) by mangling `lexical_form` into a
sense-numbered suffix (`particle` / `particle_2`) -- functionally
correct, but a genuinely ugly, confusing thing to show a user (the
Physics Domain's own `object`/`particle` conflicts,
`examples/physics_domain_seeding.py`, rendered exactly this way in
`DictionaryView`). `entry_id` replaces that: both senses now keep the
identical, unmangled `lexical_form`, and `register_conflicting_sense`
was simplified to no longer touch it at all, relying on `entry_id`
(shown directly in `DictionaryView`'s detail panel, next to the
Domain/part-of-speech pills that already distinguish the two rows) to
tell them apart instead.
`vocabulary/data/dictionary.py`'s now-unused
`next_available_lexical_form`/`find_by_lexical_form` were removed
along with the mechanism they existed to serve. Applied via a one-time
migration, `examples/word_entry_id_backfill.py`: idempotent (a second
run adds nothing), backfilled all 838 existing entries across every
mandatory, supplementary, and promoted word file with a freshly
generated UUID, verified globally unique by `WordSeeder.validate_assets()`
(now enforcing entry_id presence and uniqueness across the whole
cache, the same discipline already applied to
`(lexical_form, part_of_speech)`). Physics Domain words (hydrated at
runtime from fixture data, not loaded from a stored cache entry) get
their `entry_id` the same way they already got their `uuid`: freshly
generated by `Word`'s own `default_factory` at construction time --
no asset file exists for them to backfill. Word and relationship
counts are unchanged (838 words, 411 Physics-domain relationships);
this adds a field, it doesn't seed anything new. Verified directly:
loaded the full cache and confirmed exactly 838 unique `entry_id`
values, re-ran the full seeding chain and Playwright-checked the
Physics Domain's `object`/`particle` rows -- both now display the
plain, unmangled `lexical_form` in `DictionaryView`, distinguished by
their Domain pill and by their own `entry_id` line in the detail
panel, with no console errors.
