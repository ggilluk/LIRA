...
| Method | Responsibility |
|--------|-----------------|
| `validate_assets()` | Schema, per-file and total relationship counts, relationship kind validity, mandatory file existence, manifest consistency, manifest checksum verification, and (`asset_version 1.8.0`) that any `source_part_of_speech`/`target_part_of_speech` field present names a real `PartOfSpeech` member. |
| `load_relationship_specs()` | Validates, then parses every category file into `(source_lexical_form, source_part_of_speech, target_lexical_form, target_part_of_speech, LexicalRelationshipType)` tuples -- the two `part_of_speech` entries are `Optional[PartOfSpeech]`, `None` unless the entry sets the corresponding JSON field. Cached after the first call. |
| `seed_domain(domain)` | Resolves every relationship against `domain`'s own `Dictionary` in a complete first pass -- raising if a source or target `Word` cannot be resolved, before creating anything, so a resolution failure partway through the cache never leaves a partially-seeded relationship graph -- then creates each one not already present (same source `Word`, kind, and target `Word` -- 12.3's duplicate definition). Resolution goes through `_resolve(dictionary, lexical_form, part_of_speech)`: with no `part_of_speech`, it's a plain `Dictionary.lookup()` (first-seeded-wins by text alone, unchanged since the cache's original design); with one, it's `Dictionary.lookup_all()` filtered to the matching sense, raising if none matches -- this is what lets a cache entry name the `VERB` sense of a Common homograph like `cause` or `state` explicitly, rather than silently resolving to whichever sense happened to load first (see the relationship cache README's `asset_version 1.8.0` Version entry for the bug this fixed and the homograph-safe entries it unblocked). Every relationship it creates gets all four system properties (confidence, provenance, temporal, activation) set to `SEEDER_DEFAULT_WEIGHT` (`0.9999`) -- a seeded relationship is a curated linguistic fact ("be" → "am" is `FIRST_PERSON_FORM`), not an unweighted placeholder, so it doesn't get `LexicalRelationshipProcessor.create`'s `0.0` default. `0.9999` rather than a literal `1.0` follows the same convention the Knowledge Layer uses for directly authored facts (`knowledge/data/tensor_graph.py`): certainty is never asserted as exactly `1.0`. |
