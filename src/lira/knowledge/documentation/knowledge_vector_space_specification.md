...
| `LexicalRelationshipType` mapping (41.11) | **Built** as the mapping table itself -- `knowledge/data/lexical_relationship_type_mapping.py`'s `vector_space_dimension_for`, matching the spec's own table exactly (including `HYPERNYM`'s dual D1/D3 mapping, resolved by part of speech). Does **not** include a Vocabulary-to-Knowledge materialisation pipeline (auto-creating Concepts/edges from seeded Words) -- a separate, larger integration decision the spec's own "reinterpretation layer, not a replacement" framing doesn't require |
