...
class ObligationKind(Enum):
    ...
    (Linguistics Layer developer specification, 8.3; spec 17's own
    worked examples). A phrase/clause/sentence is not VALID while any
    obligation it raised remains undischarged (spec 17's closing line) --
    GrammarConfigurator.obligation_discharges names which PartOfSpeech
...
class SequencingObligation:
    """One instance of an obligation raised during sequencing -- e.g.
    the PREPOSITION_REQUIRES_OBJECT obligation "over" raises the moment
    it's read as the start of a PREPOSITIONAL_PHRASE, discharged once a
    NOUN_PHRASE is nested under it. raised_at_index is the token index
    that raised it, for structured-error reporting (a ReadingError's
    own unfinished_obligation field points back to one of these)."""
