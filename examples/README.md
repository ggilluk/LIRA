...
| `relationship_contradiction_audit.py` | Read-only audit of `semantic_relationships.json` for word pairs carrying more than one relationship kind at once (e.g. SYNONYM *and* HYPERNYM/HYPONYM between the same two words -- logically impossible). Found 37 contradictory pairs (74 edges), traced to the `common_semantic_completion.py` drafting pass's aggregation step never checking for a *different* kind proposed for the same pair. `relationship_contradiction_report.md` is the reviewed correction list (recommended fix + reasoning per pair, not yet applied); `relationship_contradiction_corrections.json` is the same data, structured. `python3 examples/relationship_contradiction_audit.py` from the repo root. |
