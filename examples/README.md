...
| `cause_entailment_backfill.py` | `CAUSE` is a subtype of `ENTAILMENT` (causation is a stronger claim -- if X causes Y, X's occurrence logically entails Y's -- but not every entailment is causal), the same relationship `TROPONYM` has to `HYPERNYM`/`HYPONYM`, except `CAUSE`'s companion is *same-direction*, not reversed. Backfills the missing `ENTAILMENT` edge for the 3 existing `CAUSE` pairs (`acquire`/`possess`, `attract`/`move`, `trigger`/`start`); no existing edge is touched -- see `assets/common/en/relationships/README.md`'s `asset_version 1.18.0` Version entry. `python3 examples/cause_entailment_backfill.py` from the repo root, then `python3 examples/common_semantic_completion_seeding.py` to regenerate the example UI. |
